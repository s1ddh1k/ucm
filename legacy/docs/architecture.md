...
│                                                                     │
│   ┌──────────────┐  ┌───────────────┐  ┌──────────────────┐       │
│   │ Task Queue   │  │ Forge Pipeline│  │Observer/Automation│       │
│   │ (processLoop)│──│ (ForgePipeline│  │ (ucmd-observer,  │       │
│   │              │  │  per task)    │  │  refinement auto)│       │
│   └──────────────┘  └───────────────┘  └──────────────────┘       │
│                            │                                        │
│                     ┌──────┴──────┐                                 │
│                     │ Git Worktree│                                 │
...
| `ucmd-refinement.js` | 대화형 요구사항 정제 (Q&A + refinement autopilot) |
...
```js
async function run({ taskId, dag, project, autoApprove, subtask, timeouts, onLog }) {
  // 1. 아티팩트 로드
  // 2. AI 에이전트 실행
  // 3. 결과 저장
