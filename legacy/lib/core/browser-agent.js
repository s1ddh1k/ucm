...
function normalizeProvider(provider) {
  const selected = String(
    provider ||
      process.env.UCM_BROWSER_AGENT_PROVIDER ||
      process.env.LLM_PROVIDER ||
      "codex",
  ).toLowerCase();
  if (!BROWSER_AGENT_PROVIDERS.has(selected)) {
    throw new Error(
      `unsupported browser agent provider: ${selected} (supported: claude, codex, gemini)`,
    );
  }
  return selected;
}
...
function cleanupWorkDir(workDir) {
  try {
    execSync("pkill -f chrome-devtools-mcp 2>/dev/null || true", {
      stdio: "ignore",
      timeout: 3000,
    });
  } catch (e) {
    if (e.code !== "ESRCH" && !e.killed)
      console.error(
        `[browser-agent] pkill chrome-devtools-mcp: ${e.code || e.message}`,
      );
  }
  try {
    ...
    if (e.code !== "ENOENT")
      console.error(
        `[browser-agent] rmSync ${workDir}: ${e.code || e.message}`,
      );
  }
}
...
${instruction}${jsonSuffix}`;
    ...
    const configOverrides =
      provider === "codex" ? codexMcpOverrides() : undefined;
    ...
    const result = await spawnLlm(prompt, {
      ...
      hardTimeoutMs,
      extraEnv:
        provider === "gemini" ? { GEMINI_CLI_NO_RELAUNCH: "true" } : undefined,
      onStderr: onLog
        ? (chunk) => onLog(`[browser-agent:stderr] ${chunk.trim()}`)
        : undefined,
    });
...
[{"id":"...","pass":true,"evidence":"what you observed"}, ...]`;
    ...
    if (onLog)
      onLog(
        `[browser-agent] batch: ${tasks.length} tasks, timeout ${hardTimeoutMs}ms`,
      );
    ...
    if (parsed.json && Array.isArray(parsed.json)) {
      return tasks.map((t, i) => {
        ...
        if (!r)
          return { id: t.id, pass: false, evidence: "no result returned" };
