...
async function runParallel(
  ...
  async function runInstance(id, retriesLeft) {
    ...
    if (result.status === "done") {
      if (onProgress)
        onProgress({ type: "done", id, elapsed: elapsed(instanceStart) });
      results.done.push(id);
      return "done";
    }
    if (result.status === "timeout") {
      if (onProgress)
        onProgress({ type: "timeout", id, elapsed: elapsed(instanceStart) });
    ...
    if (retriesLeft > 0) {
      if (onProgress)
        onProgress({ type: "retry", id, error: result.stderr?.slice(0, 200) });
