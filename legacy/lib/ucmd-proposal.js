...
function computeDedupHash(title, category, change) {
  const parts = [title, category, change].map((s) =>
...
function serializeProposal(proposal) {
  ...
  if (proposal.project) meta.project = proposal.project;
  if (proposal.relatedTasks && proposal.relatedTasks.length > 0) {
    meta.relatedTasks = proposal.relatedTasks;
  }
  // Curation extensions (all optional for backward compat)
  if (proposal.scores) meta.scores = proposal.scores;
  if (proposal.scoreSource) meta.scoreSource = proposal.scoreSource;
  if (proposal.weightProfile) meta.weightProfile = proposal.weightProfile;
  if (proposal.modeEligibility) meta.modeEligibility = proposal.modeEligibility;
  if (proposal.clusterId) meta.clusterId = proposal.clusterId;
  if (proposal.packagedAt) meta.packagedAt = proposal.packagedAt;
  if (proposal.packagingMode) meta.packagingMode = proposal.packagingMode;
  if (proposal.overallConfidence != null) meta.overallConfidence = proposal.overallConfidence;
  if (proposal.dimensionConfidence) meta.dimensionConfidence = proposal.dimensionConfidence;
  if (proposal.questionsAsked != null) meta.questionsAsked = proposal.questionsAsked;
  if (proposal.curationHistory && proposal.curationHistory.length > 0) {
    meta.curationHistory = proposal.curationHistory;
  }
...
function parseProposalFile(content) {
  ...
  for (let i = 0; i < sectionStarts.length; i++) {
    const end =
      i + 1 < sectionStarts.length
        ? sectionStarts[i + 1].index - sectionStarts[i + 1].name.length - 3
        : body.length;
    sections[sectionStarts[i].name] = body
      .slice(sectionStarts[i].index, end)
      .trim();
  }
  ...
  // Carry through curation extension fields from meta
  for (const key of [
    "scores", "scoreSource", "weightProfile", "modeEligibility",
    "clusterId", "packagedAt", "packagingMode", "overallConfidence",
    "dimensionConfidence", "questionsAsked", "curationHistory",
  ]) {
...
async function loadProposal(proposalId) {
  for (const status of PROPOSAL_STATUSES) {
    ...
    try {
      ...
      if (e && e.code !== "ENOENT")
        log(
          `[proposal] loadProposal read error (${proposalId}/${status}): ${e.message}`,
        );
    }
  }
  return null;
...
async function moveProposal(proposalId, fromStatus, toStatus) {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      log(
        `[proposal] moveProposal source cleanup error (${proposalId}): ${e.message}`,
      );
  }
  return proposal;
}
...
async function listProposals(statusFilter) {
  ...
  for (const status of statuses) {
    ...
    try {
      ...
      if (e && e.code !== "ENOENT")
        log(`[proposal] listProposals readdir error (${status}): ${e.message}`);
      continue;
    }
    for (const file of files) {
      ...
      try {
        ...
        if (e && e.code !== "ENOENT")
          log(`[proposal] listProposals parse error (${file}): ${e.message}`);
      }
    }
  }
...
async function deleteProposal(proposalId) {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      log(
        `[proposal] deleteProposal unlink error (${proposalId}): ${e.message}`,
      );
    return null;
  }
...
async function saveSnapshot(metrics) {
  ...
  for (let seq = 0; seq < 1000; seq++) {
    ...
    const filePath = path.join(
      SNAPSHOTS_DIR,
      `snapshot-${fileTimestamp}-${suffix}.json`,
    );
    try {
...
async function loadLatestSnapshot() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      log(`[snapshot] loadLatestSnapshot readdir error: ${e.message}`);
    return null;
  }
  ...
  try {
    return JSON.parse(
      await readFile(
        path.join(SNAPSHOTS_DIR, snapshots[snapshots.length - 1]),
        "utf-8",
      ),
    );
  } catch (e) {
    log(`[snapshot] loadLatestSnapshot parse error: ${e.message}`);
    return null;
  }
}
...
async function loadAllSnapshots() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      log(`[snapshot] loadAllSnapshots readdir error: ${e.message}`);
    return [];
  }
  const snapshots = files
    .filter((f) => f.startsWith("snapshot-") && f.endsWith(".json"))
  ...
  for (const file of snapshots) {
    try {
      const data = JSON.parse(
        await readFile(path.join(SNAPSHOTS_DIR, file), "utf-8"),
      );
      results.push(data);
    } catch (e) {
      log(`[snapshot] loadAllSnapshots parse error (${file}): ${e.message}`);
    }
  }
  return results;
}
...
async function cleanupOldSnapshots() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      log(`[snapshot] cleanupOldSnapshots readdir error: ${e.message}`);
    return;
  }
  const snapshots = files
    .filter((f) => f.startsWith("snapshot-") && f.endsWith(".json"))
  ...
  for (const file of toDelete) {
    try {
      ...
      if (e && e.code !== "ENOENT")
        log(`[snapshot] cleanup unlink error (${file}): ${e.message}`);
    }
  }
}
...
function compareSnapshots(baseline, current) {
  ...
  delta.avgPipelineDurationMs =
    current.avgPipelineDurationMs - baseline.avgPipelineDurationMs;

  if (baseline.loopMetrics && current.loopMetrics) {
    delta.firstPassRate = round2(
      (current.loopMetrics.firstPassRate || 0) -
        (baseline.loopMetrics.firstPassRate || 0),
    );
    delta.avgIterations = round2(
      (current.loopMetrics.avgIterations || 0) -
        (baseline.loopMetrics.avgIterations || 0),
    );
  }
...
async function findProposalByTaskId(taskId) {
  ...
  let files;
  try {
    files = await readdir(implementedDir);
  } catch (e) {
    if (e && e.code !== "ENOENT")
      log(`[proposal] findProposalByTaskId readdir error: ${e.message}`);
    return null;
  }
  for (const file of files) {
    if (!file.endsWith(".md")) continue;
    try {
      ...
      if (proposal.implementedBy === taskId) {
        return {
          ...proposal,
          status: "implemented",
          _filePath: path.join(implementedDir, file),
        };
      }
    } catch (e) {
      if (e && e.code !== "ENOENT")
        log(
          `[proposal] findProposalByTaskId parse error (${file}): ${e.message}`,
        );
    }
  }
  return null;
