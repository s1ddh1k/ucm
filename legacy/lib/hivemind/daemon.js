...
const _CONSOLIDATION_INTERVAL_MS = 86_400_000;
...
function enqueueUniqueItem(adapterName, item) {
  const ref = item?.ref;
  if (typeof ref !== "string" || ref.length === 0) return false;
  if (queueRefs.has(ref)) return false;
  queue.push({ adapter: adapterName, ...item });
  queueRefs.add(ref);
  return true;
}

function dequeueBatch(limit) {
  const batchSize = Number.isFinite(limit) ? Math.max(0, limit) : 0;
  if (batchSize === 0 || queue.length === 0) return [];
  const batch = queue.splice(0, batchSize);
  for (const item of batch) queueRefs.delete(item.ref);
  return batch;
}
...
function log(msg) {
  ...
  try {
    ...
    // Last-resort: stderr when log file is unwritable
    process.stderr.write(
      `[hivemind] log write failed: ${e.message} | ${line}\n`,
    );
  }
}
...
async function scanAll() {
  ...
  for (const [
    name,
    { module: adapter, config: adapterConfig },
...
async function processQueue() {
  ...
  while (queue.length > 0 && !shutdownRequested) {
    const batch = dequeueBatch(CONCURRENCY);
    const extractionResults = await Promise.all(
      batch.map((item) =>
        extractItem(item).catch((e) => {
    ...
    // Phase 1.5: Filter general knowledge
    const filterModel = config.models?.expansion; // fast model
    const provider = config.llmProvider;
    for (const result of extractionResults) {
      if (!result) continue;
      try {
        result.zettels = await extract.filterGeneralKnowledge(result.zettels, {
          model: filterModel,
          provider,
          log,
        });
      } catch (e) {
        log(`General knowledge filter error: ${e.message}`);
      }
    }
    ...
    const allSaved = [];
    for (const result of extractionResults) {
      if (!result || result.zettels.length === 0) continue;
      try {
    ...
    // Phase 2.5: Expand search terms
    const expansionModel = config.models?.expansion;
    await Promise.all(
      allSaved
        .filter((z) => !z.searchTerms?.length)
        .map((z) =>
          extract
            .expandSearchTerms(z, { model: expansionModel, provider, log })
            .catch((e) => log(`Expansion error for ${z.id}: ${e.message}`)),
        ),
    );
  }
  ...
  try {
    ...
    if (cleanupResult.bodyFixed > 0 || cleanupResult.kwFixed > 0) {
      log(
        `Cleanup: ${cleanupResult.bodyFixed} bodies deduped, ${cleanupResult.kwFixed} keywords capped`,
      );
    }
    const dedupResult = lifecycle.dedupAll({ log });
    if (dedupResult.merged > 0) {
      log(
        `Cross-dedup: ${dedupResult.merged} merges, ${dedupResult.remaining} remaining`,
      );
    }
  } catch (e) {
  ...
  // Phase 4: Recall feedback — LLM evaluates whether recalled context was useful
  try {
    await processRecallFeedback();
  } catch (e) {
    log(`Recall feedback error: ${e.message}`);
  }
...
async function processRecallFeedback() {
  if (!fs.existsSync(store.RECALL_LOG_PATH)) return;

  let lines;
  try {
    lines = fs
      .readFileSync(store.RECALL_LOG_PATH, "utf8")
      .trim()
      .split("\n")
      .filter(Boolean);
  } catch {
  ...
  // Parse recall events, group by session
  const bySession = {};
  for (const line of lines) {
    try {
      const entry = JSON.parse(line);
      if (!entry.s || !entry.ids?.length) continue;
      if (!bySession[entry.s]) bySession[entry.s] = new Set();
      for (const id of entry.ids) bySession[entry.s].add(id);
    } catch {}
  }

  // Find completed sessions (transcript exists and hasn't been modified in >30min)
  const HOME = process.env.HOME || process.env.USERPROFILE;
  const projectsDir = path.join(HOME, ".claude", "projects");
  const staleThreshold = Date.now() - 30 * 60_000;
  const feedbackModel = config.models?.expansion; // fast model
  const provider = config.llmProvider;
  let boosted = 0;
  const processedSessions = new Set();

  for (const [sessionId, zettelIds] of Object.entries(bySession)) {
    // Find transcript file matching this session
    ...
    let sessionText;
    try {
      sessionText = fs.readFileSync(transcriptPath, "utf8");
    } catch {
      continue;
    }

    // Evaluate via LLM
    const results = await extract.evaluateRecallFeedback(
      sessionText,
      [...zettelIds],
      { model: feedbackModel, provider },
    );
    for (const { id, useful } of results) {
      if (useful) {
  ...
  // Rewrite recall log, keeping only unprocessed entries
  if (processedSessions.size > 0) {
    const remaining = lines.filter((line) => {
      try {
        const entry = JSON.parse(line);
        return !processedSessions.has(entry.s);
      } catch {
        return false;
      }
    });
    try {
      const tmpPath = `${store.RECALL_LOG_PATH}.tmp`;
      fs.writeFileSync(tmpPath, remaining.length ? `${remaining.join("\n")}\n` : "");
      fs.renameSync(tmpPath, store.RECALL_LOG_PATH);
    } catch {}

    if (boosted > 0) {
      log(`Recall feedback: boosted ${boosted} zettels from ${processedSessions.size} sessions`);
    }
  }
}
...
async function ingest(adapterName) {
  ...
  try {
    ...
    const newItems = items.filter((item) => !queueRefs.has(item.ref));

    log(
      `Ingest ${adapterName}: ${newItems.length} items found (${items.length - newItems.length} already queued)`,
    );
    let totalSaved = 0;
    const extractionModel = config.models?.extraction;
    const provider = config.llmProvider;
...
async function gitCommit() {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", () => {
      const commit = spawn(
        "git",
        ["commit", "-m", `auto: ${new Date().toISOString()}`],
        {
...
function initGitRepo() {
  ...
  if (!fs.existsSync(gitDir)) {
    try {
      ...
      log(`Git repo init failed: ${e.message}`);
    }
  }
}
...
function startSocketServer() {
  if (fs.existsSync(store.SOCK_PATH)) {
    try {
      fs.unlinkSync(store.SOCK_PATH);
    } catch (e) {
      if (e.code !== "ENOENT")
        log(`Failed to remove stale socket: ${e.message}`);
    }
  }
  ...
  socketServer = net.createServer((conn) => {
    ...
    conn.on("data", (chunk) => {
      ...
      handleRequest(request)
        .then((result) => {
          try {
            conn.write(`${JSON.stringify({ ok: true, data: result })}\n`);
          } catch (writeErr) {
            log(`Socket write error (success response): ${writeErr.message}`);
          }
          conn.end();
        })
        .catch((e) => {
          try {
            ...
            log(
              `Socket write error (error response for: ${e.message}): ${writeErr.message}`,
            );
          }
          conn.end();
        });
...
async function handleRequest(request) {
  ...
  switch (method) {
    ...
    case "show": {
      ...
      const boosted = store.boostZettel(params.id);
      indexer.updateBoost(params.id);
      return boosted || zettel;
    }
    ...
    case "delete":
      if (!store.deleteZettel(params.id))
        throw new Error(`Not found: ${params.id}`);
      indexer.unindexZettel(params.id);
      return { deleted: params.id };

    case "restore": {
      if (!store.restoreZettel(params.id))
        throw new Error(`Not found in archive: ${params.id}`);
    ...
    case "link":
      if (!store.addLink(params.id1, params.id2))
    ...
    case "expand-all": {
      const allZettels = store.listZettels();
      const needExpansion = allZettels.filter((z) => !z.searchTerms?.length);
      if (needExpansion.length === 0) return { expanded: 0, total: allZettels.length };
      const expansionModel = config.models?.expansion;
      const provider = config.llmProvider;
      let expanded = 0;
      for (const z of needExpansion) {
        try {
          await extract.expandSearchTerms(z, { model: expansionModel, provider, log });
          expanded++;
        } catch (e) {
          log(`Expand error for ${z.id}: ${e.message}`);
        }
      }
      return { expanded, total: allZettels.length };
    }
...
function startTimers() {
  timers.push(
    setInterval(() => {
      scanAll()
        .then(processQueue)
        .catch((e) => log(`Scan/process error: ${e.message}`));
    }, SCAN_INTERVAL_MS),
  );
...
function gracefulShutdown() {
  ...
  if (socketServer) {
    ...
    try {
      ...
      if (e.code !== "ENOENT")
        log(`Failed to remove socket on shutdown: ${e.message}`);
    }
  }

  try {
    fs.unlinkSync(store.PID_PATH);
  } catch (e) {
    if (e.code !== "ENOENT")
      log(`Failed to remove PID file on shutdown: ${e.message}`);
  }
  indexer.close();
...
async function startDaemon(foreground) {
  ...
  if (!foreground) {
    ...
    console.log(
      `hivemind daemon started (pid: ${child.pid}) — socket may be slow`,
    );
    return;
  }
  ...
  log(
    `Index loaded: ${indexStats.zettels} zettels, ${indexStats.keywords} keywords`,
  );
  ...
  scanAll()
    .then(processQueue)
    .catch((e) => log(`Initial scan error: ${e.message}`));
}
...
function stopDaemon() {
  ...
  const pid = parseInt(fs.readFileSync(store.PID_PATH, "utf8").trim(), 10);
  try {
    ...
    if (e.code === "ESRCH") {
      ...
      try {
        ...
        if (e.code !== "ENOENT")
          console.error(`Failed to remove stale PID file: ${e.message}`);
      }
    } else {
      throw e;
    }
...
module.exports = {
  ...
  _test: {
    ...
    getQueueRefCount: () => queueRefs.size,
  },
};
