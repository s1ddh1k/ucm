...
4. **에피소드** (type: "episode"): 다음 세션에서 이어받을 때 필요한 맥락만 기록 (소스가 대화 로그일 때만, 최대 1개).
   - "무엇을 했는지" 나열 금지. "어디까지 했고 다음에 뭘 해야 하는지"만 3줄 이내로.
   - 완결된 작업(리뷰 완료, 배포 완료, 버그 수정 완료 등)은 에피소드로 추출하지 마라.
...
const GENERAL_KNOWLEDGE_PROMPT = `이 노트가 특정 프로젝트/팀/환경에 고유한 지식인지, AI 모델이 이미 아는 일반 지식인지 판단하라.
...
판정 기준:
- SPECIFIC: 프로젝트명, 팀 컨벤션, 고유 설정, 개인 패턴, 디버깅 결과, 비문서화된 동작 등 맥락 없이는 알 수 없는 것
- GENERAL: 공식 문서 내용, 프레임워크 기본 사용법, 디자인 패턴, 언어 문법 등 AI 모델 학습 데이터에 포함된 것

**주의**: 일반 기술을 특정 프로젝트에 적용한 기록(예: "우리 프로젝트에서 Spring 프로필을 이렇게 구성했다")은 SPECIFIC이다.

JSON만 출력: {"verdict": "SPECIFIC|GENERAL"}
`;

async function filterGeneralKnowledge(zettels, { model, provider, log } = {}) {
  const logFn = log || (() => {});
  const results = [];

  for (const z of zettels) {
    const kwList = Object.keys(z.keywords || {}).join(", ");
    const prompt = GENERAL_KNOWLEDGE_PROMPT
      .replace("{TITLE}", z.title || "")
      .replace("{KEYWORDS}", kwList)
      .replace("{BODY}", (z.body || "").slice(0, 500));

    try {
      const result = await callLlmJson(prompt, { model, provider });
      if (result?.verdict === "GENERAL") {
        logFn(`  General knowledge filtered: "${(z.title || "").slice(0, 50)}"`);
        continue;
      }
    } catch {
      // On LLM failure, keep the zettel (safe default)
    }
    results.push(z);
  }
...
// --- Recall feedback evaluation ---

const RECALL_FEEDBACK_PROMPT = `너는 기억 시스템의 품질 평가자다.
에이전트 세션 시작 시 자동으로 주입된 기억들이 실제 대화에서 유용했는지 판단하라.

## 주입된 기억 목록
각 <memory>는 세션에 주입된 하나의 기억이다. keywords는 인출을 유발한 검색 키워드, body는 주입된 본문이다.
...
## 판정 기준
각 기억에 대해 대화를 읽고 판단하라:
- useful=true: 대화에서 해당 기억의 지식이 **실제로 활용**된 경우. 예: 기억의 내용이 코드·설정·답변·의사결정에 반영됨, 사용자가 기억의 주제를 직접 언급/질문함
- useful=false: 주입되었으나 대화 흐름과 무관하거나 완전히 무시된 경우

**주의**: keywords가 대화에 등장하는 것만으로는 useful이 아니다. 기억의 body 내용이 대화에 실질적 영향을 줬는지 봐라.

JSON만 출력: {"evaluations": [{"id": "...", "useful": true/false}]}
`;

async function evaluateRecallFeedback(sessionText, recalledIds, { model, provider } = {}) {
  const zettels = [];
  for (const id of recalledIds) {
    const z = store.loadZettel(id);
    if (!z) continue;
    zettels.push({
      id,
      title: z.title,
      type: z.memoryType || z.kind || "unknown",
      keywords: Object.keys(z.keywords || {}),
      body: (z.body || "").slice(0, 400),
    });
  }
  if (zettels.length === 0) return [];

  const zettelBlock = zettels
    .map(
      (z, i) =>
        `<memory index="${i + 1}" id="${z.id}">\n` +
        `  <title>${z.title}</title>\n` +
        `  <type>${z.type}</type>\n` +
        `  <keywords>${z.keywords.join(", ")}</keywords>\n` +
        `  <body>${z.body}</body>\n` +
        `</memory>`,
    )
    .join("\n\n");

  // Session excerpt: skip first 2000 chars (likely contains the injected context itself), take next 6000
  const excerpt = sessionText.slice(2000, 8000);

  const prompt = RECALL_FEEDBACK_PROMPT
    .replace("{RECALLED_ZETTELS}", zettelBlock)
  ...
  try {
    ...
    return evals
      .filter((e) => e?.id && typeof e.useful === "boolean")
      .map((e) => ({ id: e.id, useful: e.useful }));
  } catch {
    return [];
  }
}
...
const EXPANSION_PROMPT = `이 노트를 검색할 때 사용할 수 있는 모든 표현을 생성하라.
...
규칙:
- 동의어, 약어, 한/영 번역
- 사용자가 자연어로 이 주제를 언급할 때 쓸 법한 표현
- 관련 기술 용어
- 각 term은 1-2 단어, 최대 20개
- 이미 키워드에 있는 것은 제외

JSON: {"terms": ["term1", "term2", ...]}
`;

async function expandSearchTerms(zettel, { model, provider, log } = {}) {
  const logFn = log || (() => {});
  const keywordList = Object.keys(zettel.keywords || {});
  const prompt = EXPANSION_PROMPT.replace("{TITLE}", zettel.title || "")
    .replace("{KEYWORDS}", keywordList.join(", "));

  let result;
  try {
    result = await callLlmJson(prompt, { model, provider });
  } catch (e) {
    logFn(`Expansion LLM failed for ${zettel.id}: ${e.message}`);
    return;
  }
  ...
  // Filter out terms already in keywords
  const existingKws = new Set(keywordList.map((k) => k.toLowerCase()));
  const newTerms = terms.filter((t) => !existingKws.has(t));
  if (newTerms.length === 0) return;

  // Save to index
  indexer.indexExpansions(zettel.id, newTerms);

  // Save to zettel file for reindex persistence
  zettel.searchTerms = newTerms;
  store.saveZettel(zettel);

  logFn(`  Expanded ${zettel.id}: +${newTerms.length} search terms`);
}
...
function bodyOverlap(existingBody, newBody) {
  const existingParas = existingBody
    .split(/\n{2,}/)
    .map((p) => p.replace(/^---$/, "").trim())
    .filter((p) => p.length > 30);
  const newParas = newBody
    .split(/\n{2,}/)
    .map((p) => p.replace(/^---$/, "").trim())
    .filter((p) => p.length > 30);
...
async function extractZettels(text, source, { log, model, provider } = {}) {
  ...
  const prompt =
    EXTRACTION_PROMPT_PRE + text.slice(0, 80_000) + EXTRACTION_PROMPT_POST;
  let rawZettels;
  try {
  ...
  for (const raw of rawZettels) {
    ...
    if (coreCount < 3) {
      const fallbackWords = extractKeywordsFromText(
        raw.title || "",
        raw.body || "",
      );
      let added = 0;
      ...
      if (added > 0) {
        logFn(
          `  Keyword supplement for "${(raw.title || "").slice(0, 40)}": +${added} from text`,
        );
      }
    }
...
function deduplicateBatch(zettels, { log } = {}) {
  ...
  for (let i = 0; i < zettels.length; i++) {
    ...
    for (let j = i + 1; j < zettels.length; j++) {
      ...
      const coreOverlap = [...survivorCore].filter((k) =>
        candidateCore.has(k),
      ).length;
      ...
      if (titleSim >= 0.4 && coreOverlap >= 1) {
        ...
        const keep =
          (survivor.body || "").length >= (candidate.body || "").length
            ? survivor
            : candidate;
        ...
        logFn(
          `  Intra-dedup: "${discard.title.slice(0, 50)}" → "${keep.title.slice(0, 50)}"`,
        );
        removed.add(j);
        if (keep !== survivor) {
          removed.add(i);
  ...
  if (removed.size > 0) {
    logFn(
      `Intra-batch dedup: ${zettels.length} → ${kept.length} (removed ${removed.size})`,
    );
  }
  return kept;
}
...
async function checkDuplicate(newZettel, { model, provider } = {}) {
  ...
  const newAllKws = new Set(
    Object.keys(newZettel.keywords || {}).map((k) => k.toLowerCase()),
  );
  const candidates = [];
  for (const entry of allEntries) {
    ...
    if (coreOverlap.length >= 1) {
      candidates.push({
        id: entry.id,
        title: entry.title,
        overlapRatio: coreOverlap.length / newCoreKws.size,
      });
      continue;
    }
    // Cross-implementation detection: different core keywords but many shared context keywords
    const entryAllKws = new Set(
      Object.keys(entry.keywords || {}).map((k) => k.toLowerCase()),
    );
    ...
    if (allOverlap.length >= 3) {
      candidates.push({
        id: entry.id,
        title: entry.title,
        overlapRatio: allOverlap.length / newAllKws.size,
      });
    }
  }
  ...
  const prompt = DEDUP_PROMPT.replace("{EXISTING_TITLE}", existing.title)
...
async function processAndSave(
  zettels,
  { log, skipDedup = false, model, dedupModel, provider } = {},
) {
  const logFn = log || (() => {});
  const saved = [];
  ...
  for (const zettel of zettels) {
    ...
    if (!skipDedup) {
      ...
      if (dedup.verdict === "MERGE") {
        logFn(
          `  MERGE: "${zettel.title.slice(0, 50)}" → ${dedup.existingId} (${dedup.reason || ""})`,
        );
      ...
      if (dedup.verdict === "SUPERSEDE") {
        logFn(
          `  SUPERSEDE: "${zettel.title.slice(0, 50)}" replaces ${dedup.existingId}`,
        );
...
module.exports = {
  extractZettels,
  expandSearchTerms,
  evaluateRecallFeedback,
  filterGeneralKnowledge,
  deduplicateBatch,
  checkDuplicate,
  generateLinks,
  processAndSave,
