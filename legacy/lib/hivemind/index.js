...
const { search, decayScore } = require("./search");
...
function printSearchResults(results) {
  ...
  for (const r of results) {
    ...
    const titleLine = `${r.id}  ${(r.score || 0).toFixed(4)}  ${r.title}`;
...
function printZettel(zettel) {
  ...
  if (zettel.kind) console.log(`kind: ${zettel.kind}`);
...
async function cmdInit() {
  ...
  // Setup UserPromptSubmit hook for auto-recall
  const recallHookCmd = "hm recall-hook";
  const promptSubmitEntries = settings.hooks?.UserPromptSubmit || [];
  const hasRecallHook = promptSubmitEntries.some((entry) =>
    entry.hooks?.some((h) => h.command === recallHookCmd),
  );
  if (hasRecallHook) {
    console.log("UserPromptSubmit 자동 인출 훅이 이미 설정되어 있습니다.");
  } else {
    const addRecall = await promptYesNo(
      "매 메시지마다 관련 지식을 자동 인출할까요?",
      true,
    );
    if (addRecall) {
      if (!settings.hooks) settings.hooks = {};
      ...
      settings.hooks.UserPromptSubmit.push({
        matcher: "",
        hooks: [{ type: "command", command: recallHookCmd }],
      });
      fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
      fs.writeFileSync(settingsPath, `${JSON.stringify(settings, null, 2)}\n`);
      console.log("  UserPromptSubmit 훅 등록 완료");
    }
  }
...
function cmdRecallHook() {
  let input;
  try {
    input = JSON.parse(fs.readFileSync("/dev/stdin", "utf8").trim() || "{}");
  } catch {
    return;
  }
  ...
  // Quick filter: too short
  if (prompt.length < 15) return;

  // Quick filter: affirmative/simple responses
  if (/^(네|ㅇ|ㅇㅇ|okay|yes|y|좋아|ㄱ|고|확인|ok|ㅇㅋ|알겠|응)/i.test(prompt.trim())) return;

  indexer.loadFromDisk();
  const config = store.loadConfig();

  // Tokenize and filter against known terms (dedicated SQL queries, no full entry load)
  const terms = indexer.tokenize(prompt);
  const allKeywords = indexer.getAllKeywordTerms();
  const expansionTerms = indexer.getAllExpansionTerms();
  const allKnownTerms = new Set([...allKeywords, ...expansionTerms]);

  const searchTerms = terms.filter((t) => t.length >= 2 && allKnownTerms.has(t));
  if (searchTerms.length === 0) return;

  // Cue overload filter: discard terms that match too many zettels (IDF-like)
  const totalZettels = indexer.getKeywordCount() || 1;
  const filteredTerms = searchTerms.filter((t) => {
    const hits = indexer.lookupKeyword(t);
    return hits.length <= Math.max(totalZettels * 0.3, 10);
  });
  if (filteredTerms.length === 0) return;

  // Fast search (no LLM expansion)
  const results = indexer.lookupKeywordsWeighted(filteredTerms);
  if (results.length === 0) return;

  // Apply temporal decay — recent memories surface more easily
  const scored = [];
  for (const r of results.slice(0, 20)) {
    ...
    scored.push({ id: r.id, score: finalScore, entry });
  }
  scored.sort((a, b) => b.score - a.score);

  const threshold = 0.5;
  const relevant = scored.filter((r) => r.score >= threshold).slice(0, 5);
  if (relevant.length === 0) return;

  // Session dedup
  const sessionId = input.session_id || "default";
  const seenPath = path.join(os.tmpdir(), `hm-recall-${sessionId}.seen`);
  let seenIds = new Set();
  try {
    const seenData = fs.readFileSync(seenPath, "utf8").trim();
    if (seenData) seenIds = new Set(seenData.split("\n"));
  } catch {}

  const unseen = relevant.filter((r) => !seenIds.has(r.id));
  if (unseen.length === 0) return;

  // Load bodies and build context
  const top = unseen.slice(0, 3);
  const contextParts = [];
  for (const r of top) {
    const zettel = store.loadZettel(r.id);
    if (!zettel) continue;
    const body = (zettel.body || "").slice(0, 500);
    contextParts.push(`### ${zettel.title}\n${body}`);
    seenIds.add(r.id);
  }
  ...
  // Update seen file
  try {
    fs.writeFileSync(seenPath, [...seenIds].join("\n"));
  } catch {}

  // Log recall event for feedback evaluation
  try {
    ...
    fs.appendFileSync(store.RECALL_LOG_PATH, `${logEntry}\n`);
  } catch {}

  const context = `## Recalled knowledge\n\n${contextParts.join("\n\n")}`;
...
async function cmdExpand(opts) {
  ...
  if (isDaemonRunning() && !dryRun) {
    const client = createClient();
    const result = await client({ method: "expand-all", params: {} });
    console.log(`Expanded: ${result.expanded}/${result.total} zettels`);
    return;
  }
  ...
  if (needExpansion.length === 0) {
    console.log("All zettels already have search terms.");
    return;
  }

  console.log(`${needExpansion.length}/${allZettels.length} zettels need expansion.`);
  if (dryRun) {
    for (const z of needExpansion.slice(0, 10)) {
      const kws = Object.keys(z.keywords || {}).slice(0, 5).join(", ");
      console.log(`  ${z.id}  ${z.title}  [${kws}]`);
    }
    if (needExpansion.length > 10) console.log(`  ... and ${needExpansion.length - 10} more`);
    return;
  }
  ...
  let expanded = 0;
  const BATCH = 3;
  for (let i = 0; i < needExpansion.length; i += BATCH) {
    const batch = needExpansion.slice(i, i + BATCH);
    const results = await Promise.allSettled(
      batch.map((z) =>
        extract.expandSearchTerms(z, {
    ...
    for (const r of results) {
      if (r.status === "fulfilled") expanded++;
    }
    console.log(`  progress: ${Math.min(i + BATCH, needExpansion.length)}/${needExpansion.length}`);
  }
  console.log(`Expanded: ${expanded}/${needExpansion.length} zettels`);
}
...
Usage:
  ...
  hm expand [--dry-run]           Expand search terms for all zettels
...
async function run() {
  ...
  try {
    switch (command) {
      ...
      case "expand":
        await cmdExpand({ dryRun: !!flags["dry-run"] });
        break;
