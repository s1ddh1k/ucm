...
function log(message) {
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      process.stderr.write(`[watchdog] log write error: ${e.message}\n`);
  }
...
function healthCheck() {
  return new Promise((resolve) => {
    ...
    const client = net.createConnection(SOCK_PATH, () => {
      ...
      client.write(
        `${JSON.stringify({ id: "hc", method: "stats", params: {} })}\n`,
      );
      let data = "";
...
async function run() {
  ...
  const handleChildExit = async (code) => {
    ...
    if (!shuttingDown && code !== 0) {
      ...
      try {
        await unlink(WATCHDOG_PID_PATH);
      } catch (e) {
  ...
  process.on("SIGTERM", async () => {
    ...
    try {
      ...
      if (e.code !== "ENOENT")
        log(`unlink watchdog pid on SIGTERM: ${e.message}`);
    }
    process.exit(0);
  });
  ...
  process.on("SIGINT", async () => {
    ...
    try {
      ...
      if (e.code !== "ENOENT")
        log(`unlink watchdog pid on SIGINT: ${e.message}`);
    }
    process.exit(0);
  });
}
