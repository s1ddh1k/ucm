...
const LOOPBACK_IPV4_RE = /^127(?:\.(?:25[0-5]|2[0-4]\d|1?\d?\d)){3}$/;
...
const PROXY_ROUTES = [
  ...
  },
  {
    ...
    post: true,
    method: "approve",
    params: (_, m, body) => ({ ...(body || {}), taskId: m[1] }),
  },
  {
  ...
  },
  {
    ...
    params: (_, m, body) => ({ ...(body || {}), proposalId: m[1] }),
  },
  // Curation: mode
  { pattern: /^\/api\/curation\/mode$/, method: "curation_mode" },
  {
    pattern: /^\/api\/curation\/mode$/,
    post: true,
    method: "curation_set_mode",
    bodyParams: true,
  },
  // Curation: weights
  { pattern: /^\/api\/curation\/weights$/, method: "curation_weights" },
  {
    pattern: /^\/api\/curation\/weights$/,
    post: true,
  ...
  // Proposal scoring
  {
    pattern: /^\/api\/proposal\/score\/(p-[a-f0-9]+)$/,
    method: "proposal_score",
    params: (url, m) => ({
      proposalId: m[1],
      recompute: url.searchParams.get("recompute") === "1",
    }),
  },
  {
    pattern: /^\/api\/proposal\/score\/(p-[a-f0-9]+)$/,
  ...
  {
    pattern: /^\/api\/proposal\/cluster\/merge$/,
    post: true,
    method: "proposal_cluster_merge",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/proposal\/cluster\/split\/(p-[a-f0-9]+)$/,
    post: true,
  ...
  // Conflicts
  {
    pattern: /^\/api\/proposal\/conflicts\/(p-[a-f0-9]+)$/,
    method: "proposal_conflicts",
    params: (_, m) => ({ proposalId: m[1] }),
  },
  // Discard
  {
    pattern: /^\/api\/proposal\/discard\/(p-[a-f0-9]+)$/,
    post: true,
    method: "proposal_discard",
    params: (_, m, body) => ({ ...(body || {}), proposalId: m[1] }),
  },
  {
    pattern: /^\/api\/proposal\/discard-history$/,
  ...
  // Readiness
  {
    pattern: /^\/api\/proposal\/readiness\/(p-[a-f0-9]+)$/,
  ...
  // Feedback
  {
    pattern: /^\/api\/proposal\/feedback\/(p-[a-f0-9]+)$/,
    post: true,
    method: "proposal_feedback",
    params: (_, m, body) => ({ ...(body || {}), proposalId: m[1] }),
  },
  // Scoring profile
  { pattern: /^\/api\/curation\/profiles$/, method: "scoring_profile" },
  {
...
let config = null;
const availableCliCache = new Map();
...
function normalizePtySize(cols, rows) {
  return {
    cols: clampInt(cols, 80, MIN_PTY_COLS, MAX_PTY_COLS),
    rows: clampInt(rows, 24, MIN_PTY_ROWS, MAX_PTY_ROWS),
  };
}
...
function sendPtyError(ws, message) {
  try {
    ...
    ws.send(JSON.stringify({ event: "pty:error", data: { message } }));
  } catch {}
}

async function ensureProviderCli(providerCmd) {
  if (availableCliCache.get(providerCmd) === true) return;
  await execFileAsync("which", [providerCmd], { timeout: 3000 });
  availableCliCache.set(providerCmd, true);
}

async function resolvePtyCwd(cwd) {
  if (cwd == null || cwd === "") return os.homedir();
  if (typeof cwd !== "string") throw new Error("cwd must be a string");

  const trimmed = cwd.trim();
  if (!trimmed) return os.homedir();
  if (trimmed.length > MAX_PTY_CWD_LENGTH) throw new Error("cwd is too long");
  if (trimmed.includes("\0")) throw new Error("cwd contains invalid characters");

  const resolved = path.resolve(resolveHomePath(trimmed));
  let stats;
  try {
    stats = await stat(resolved);
  } catch (e) {
    if (e.code === "ENOENT") throw new Error(`cwd does not exist: ${resolved}`);
    throw new Error(`failed to access cwd: ${e.message}`);
  }
  if (!stats.isDirectory()) throw new Error(`cwd is not a directory: ${resolved}`);
  return resolved;
}

async function spawnPtySession(ws, { cols, rows, cwd, newSession } = {}) {
  // Kill existing PTY session for this ws to prevent orphaned processes
  killPtySession(ws);

  if (getPtySessionCount() >= MAX_PTY_SESSIONS) {
    sendPtyError(ws, "max PTY sessions reached");
    return;
  }
  ...
  let resolvedCwd;
  try {
    resolvedCwd = await resolvePtyCwd(cwd);
  } catch (e) {
    sendPtyError(ws, e.message);
    return;
  }
  ...
  try {
    await ensureProviderCli(providerConfig.cmd);
  } catch {
    sendPtyError(
      ws,
      `CLI not found: ${providerConfig.cmd}. Install it or switch to a different provider.`,
    );
    return;
  }

  try {
    const systemPrompt = await buildSystemPrompt(resolvedCwd);
...
function isLoopbackAddress(address) {
  ...
  const mapped = normalized.startsWith("::ffff:")
    ...
    : normalized;
  return LOOPBACK_IPV4_RE.test(mapped);
}
...
async function startUiServer(opts = {}) {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (!isLoopbackAddress(remoteAddress)) {
      jsonResponse(res, { error: "access denied: loopback only" }, 403);
      return;
    }
    ...
    if (pathname.startsWith("/api/")) {
      const origin =
        req?.headers && typeof req.headers.origin === "string"
          ? req.headers.origin
          : "";
      if (!isAllowedApiOrigin(origin)) {
        jsonResponse(res, { error: "origin not allowed" }, 403);
        return;
      }
    ...
    try {
      ...
      // Hivemind daemon management
      if (req.method === "POST" && pathname === "/api/hivemind/start") {
        try {
          spawnHmd();
          jsonResponse(res, { ok: true, running: isHmdRunning() });
        } catch (e) {
          jsonResponse(res, { error: e.message }, 500);
        }
        return;
      ...
      if (req.method === "POST" && pathname === "/api/hivemind/stop") {
        try {
          stopHmd();
          jsonResponse(res, { ok: true, running: isHmdRunning() });
        } catch (e) {
      ...
      if (pathname === "/api/browse") {
        ...
        const { resolved, allowed } =
          await resolvePathWithinHome(requestedPath);
        if (!allowed) {
          jsonResponse(
            res,
