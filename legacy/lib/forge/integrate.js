...
async function run({ taskId, dag, project, timeouts, onLog = () => {} } = {}) {
  ...
  if (failedTasks.length > 0) {
    ...
    onLog(
      `[integrate] warning: ${failedTasks.length} subtask(s) failed: ${failedIds}`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      error.message.includes("merge failed")
    ) {
      onLog(
        "[integrate] merge conflict detected, attempting LLM resolution...",
      );

      try {
        const originPath = workspace.projects?.[0]?.origin || project;
        await resolveConflicts(taskId, originPath, {
          effectiveTimeouts,
          onLog,
        });
        ...
        onLog(
          `[integrate] conflict resolution failed: ${resolveError.message}`,
        );
  ...
  const testResult = await spawnAgent(
    ...
    {
      cwd: project,
      model: typeof STAGE_MODELS.verify === "object" ? STAGE_MODELS.verify.test : STAGE_MODELS.verify,
