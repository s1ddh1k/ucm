...
async function run({ taskId, dag, project, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      onLog(`[specify] loadArtifact error (decisions.json): ${e.message}`);
    try {
      decisionsContent = await loadArtifact(taskId, "decisions.md");
      decisions = parseDecisionsFile(decisionsContent);
      onLog(
        "[specify] using fallback artifact: decisions.md instead of decisions.json",
      );
    } catch (e2) {
      if (e2.code !== "ENOENT")
        onLog(`[specify] loadArtifact error (decisions.md): ${e2.message}`);
      onLog("[specify] no decisions artifact available");
    }
  }
  ...
  if (decisions.length === 0) {
    try {
      ...
      onLog("[specify] using fallback: task.md as decisions source");
    } catch (e) {
      if (e.code !== "ENOENT")
        onLog(`[specify] loadArtifact error (task.md): ${e.message}`);
      onLog("[specify] warning: no decisions or task artifact available");
    }
  }
  ...
  let specContent;
  if (useRsa) {
    const rsaResult = await generateWithRsa(decisions, {
      project,
      provider,
      taskId,
      onLog,
    ...
    const simpleResult = await generateSimple(decisions, {
      project,
      provider,
      onLog,
    });
  ...
  for (let attempt = 0; attempt < MAX_VALIDATION_RETRIES; attempt++) {
    ...
    try {
      validationResult = await validateRequirements(specContent, {
        cwd: project,
        provider,
      });
    ...
    const gaps = Array.isArray(validationResult.data.gaps)
      ? validationResult.data.gaps
      : [];
    onLog(`[specify] validation failed: ${gaps.length} gaps`);
    await saveArtifact(
      taskId,
      `gap-report-${attempt + 1}.md`,
      formatGapReport(gaps),
    );

    if (attempt < MAX_VALIDATION_RETRIES - 1) {
      onLog("[specify] regenerating spec with gap feedback...");
      const gapFeedback = gaps
        .map((g) => `- ${g.criterion}: ${g.detail}`)
        .join("\n");
      const regenResult = await regenerateWithFeedback(
        specContent,
        gapFeedback,
        { project, provider },
      );
      ...
      onLog(
        "[specify] max validation retries reached, proceeding with warnings",
      );
      dag.warnings.push(
        `spec validation failed after ${MAX_VALIDATION_RETRIES} attempts`,
      );
    }
  }
...
async function generateWithRsa(
  decisions,
  { project, provider, taskId, onLog },
) {
...
${decisionsText}`;
  ...
  const parallelResult = await runParallel(specPrompt, {
    ...
    onProgress: (event) => {
      if (event.type === "done")
        onLog(`[specify] worker ${event.id} done (${event.elapsed})`);
      else if (event.type === "failed")
        onLog(`[specify] worker ${event.id} failed`);
    },
  });
  ...
  for (const id of parallelResult.done) {
    try {
      ...
      if (e.code !== "ENOENT")
        onLog(`[specify] failed to read worker output ${id}: ${e.message}`);
    }
  }
  ...
  const inlineResults = workerOutputs
    .map((w) => `### Worker ${w.id}\n\n${w.content}`)
    .join("\n\n---\n\n");
...
5. 최종 결과는 하나의 일관된 마크다운 문서로 작성하세요.`;
  ...
  return {
    ...
    tokenUsage: {
      ...
      output:
        (parallelResult.tokenUsage?.output || 0) + (tokenUsage?.output || 0),
    },
  };
}
