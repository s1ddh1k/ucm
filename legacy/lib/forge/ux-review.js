...
function parseUxReview(output) {
  try {
    ...
    return {
      ...
      canUserAccomplishGoal: review.canUserAccomplishGoal || {
        goal: "",
        result: "no",
        blockers: [],
      },
      usabilityIssues: Array.isArray(review.usabilityIssues)
        ? review.usabilityIssues
        : [],
      confusingElements: Array.isArray(review.confusingElements)
        ? review.confusingElements
        : [],
    ...
    console.error(
      `[ux-review] failed to parse review output as JSON: ${e.message}`,
    );
    return {
      score: 0,
      ...
      usabilityIssues: [
        {
          severity: "critical",
          description: "Review output was not valid JSON",
          where: "",
...
function formatUxFeedback(review) {
  ...
  const critical = review.usabilityIssues.filter(
...
async function run({
  ...
  try {
    ...
    let spec = "";
    try {
      spec = await loadArtifact(taskId, "spec.md");
    } catch (e) {
      if (e.code !== "ENOENT")
        onLog(`[ux-review] loadArtifact error (spec.md): ${e.message}`);
      try {
        spec = await loadArtifact(taskId, "task.md");
        onLog(
          "[ux-review] using fallback artifact: task.md instead of spec.md",
        );
      } catch (e2) {
        if (e2.code !== "ENOENT")
          onLog(`[ux-review] loadArtifact error (task.md): ${e2.message}`);
    ...
    let design = "";
    try {
      design = await loadArtifact(taskId, `design${artifactSuffix}.md`);
    } catch (e) {
      if (e.code !== "ENOENT")
        onLog(
          `[ux-review] loadArtifact error (design${artifactSuffix}.md): ${e.message}`,
        );
      try {
        ...
        if (e2.code !== "ENOENT")
          onLog(`[ux-review] loadArtifact error (design.md): ${e2.message}`);
    ...
    onLog(
      `[ux-review] spawning UX review agent (dev: ${devServer.url}, chrome: port ${browser.port})`,
    );
    ...
    await saveArtifact(
      taskId,
      `ux-review${artifactSuffix}.json`,
      JSON.stringify(review, null, 2),
    );

    // 8. Pass/fail decision
    const criticalCount = review.usabilityIssues.filter(
      (i) => i.severity === "critical",
    ).length;
    ...
    onLog(
      `[ux-review] result: ${passed ? "PASS" : "FAIL"} (score: ${review.score}/10, critical: ${criticalCount})`,
    );
