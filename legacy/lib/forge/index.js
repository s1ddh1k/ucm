...
const RETRYABLE_FORGE_ERROR_CODES = new Set([
  "EAGAIN",
  "EBUSY",
  "ETIMEDOUT",
  "ECONNRESET",
...
function classifyPipelineError(error) {
  ...
  const retryable =
    ...
    RETRYABLE_FORGE_ERROR_CODES.has(error?.code) ||
    /(rate.?limit|429|temporar|timeout|busy|locked|eagain)/i.test(message);
  return {
...
class ForgePipeline extends EventEmitter {
  ...
  async run() {
    ...
    try {
      ...
      for (const stageName of stages) {
        ...
        const result = await this.runStage(stageName);
        ...
        // polish 후 미해결 이슈가 있으면 피드백 루프 실행
        if (stageName === "polish" && result?.hasUnresolvedIssues && result.feedback && !this.aborted) {
          await this.runPolishFeedbackIteration(null, result.feedback);
        }
      }
      ...
      const currentStage = this.dag?.currentStage || "unknown";
      ...
      this.dag.warnings.push(
        `${errorClass.kind} pipeline error (taskId: ${this.taskId}, stage: ${currentStage}, code: ${errorClass.code || "none"}): ${error.message}`,
      );
      try {
        await this.dag.save();
      } catch (saveErr) {
  ...
  async runStage(stageName, { subtask } = {}) {
    ...
    try {
      ...
      // medium/large 파이프라인에서 verify review를 opus로 업그레이드
      if (stageName === "verify") {
        stageOpts.modelOverride = this._getVerifyModelOverride();
      }

      const result = await stageModule.run(stageOpts);
      ...
      for (const subtaskId of pending) {
        ...
        try {
          ...
          if (pipelineStages.includes("polish")) {
            if (this.aborted) break;
            const polishResult = await this.runStage("polish", { subtask });
            await this.waitForStageGate("polish");
            if (polishResult?.hasUnresolvedIssues && polishResult.feedback && !this.aborted) {
              await this.runPolishFeedbackIteration(subtask, polishResult.feedback);
            }
          }
    ...
    for (let iteration = 1; iteration <= maxIterations; iteration++) {
      ...
      try {
        ...
        verifyResult = await verify.run({
          ...
          modelOverride: this._getVerifyModelOverride(),
          onLog: (line) =>
  ...
  _getVerifyModelOverride() {
    const pipeline = this.dag.pipeline;
    if (pipeline === "medium" || pipeline === "large") {
      const defaults = STAGE_MODELS.verify;
      return {
        test: (typeof defaults === "object" ? defaults.test : defaults) || "sonnet",
        review: "opus",
      };
    }
    return undefined;
  ...
  async runPolishFeedbackIteration(subtask, polishFeedback) {
    this.emit("agent:output", {
      stage: "polish",
      chunk: "[polish] running feedback iteration (implement + verify)...",
    });
    ...
    try {
      await saveArtifact(
        this.taskId,
        `polish-feedback${artifactSuffix}.md`,
        polishFeedback,
      );
    } catch (e) {
      this.emit("agent:output", {
        stage: "polish",
        chunk: `[polish] failed to save feedback artifact (non-fatal): ${e.message}`,
      });
    }
    ...
    if (this.tokenBudget > 0) {
      const used = this.dag.totalTokens();
      if (used / this.tokenBudget >= 0.95) {
        this.emit("agent:output", {
          stage: "polish",
          chunk: "[polish] skipping feedback iteration — token budget 95% reached",
        });
        return;
      }
    }

    // implement: polish 피드백을 feedback으로 전달
    const implStart = Date.now();
    try {
      this.emit("stage:start", { stage: "implement", model: null });

      const implement = require("./implement");
      const implResult = await implement.run({
        taskId: this.taskId,
        dag: this.dag,
        project: stageProject,
        autoApprove: this.autoApprove,
      ...
      this.emit("agent:output", {
        stage: "polish",
        chunk: `[polish] feedback implement failed (non-fatal): ${e.message}`,
      });
      this.dag.warnings.push(`polish feedback implement failed: ${e.message}`);
      return;
    }
    ...
    try {
      ...
      try {
        await saveArtifact(
          this.taskId,
          `verify-polish-feedback${artifactSuffix}.json`,
          JSON.stringify({
            passed: verifyResult.passed,
            feedback: verifyResult.feedback,
            report: verifyResult.report,
      ...
      if (!verifyResult.passed) {
        this.emit("agent:output", {
          stage: "polish",
          chunk: `[polish] feedback verify failed (non-fatal): ${verifyResult.feedback?.slice(0, 200) || "verification failed"}`,
        });
        this.dag.warnings.push("polish feedback verify failed");
      }
    } catch (e) {
      const verifyDuration = Date.now() - verifyStart;
      this.dag.recordStage("verify", "fail", verifyDuration, null);
      ...
      this.emit("agent:output", {
        stage: "polish",
        chunk: `[polish] feedback verify failed (non-fatal): ${e.message}`,
      });
      this.dag.warnings.push(`polish feedback verify failed: ${e.message}`);
    }
  }
  ...
  async learnToHivemind() {
    try {
      ...
      // LLM으로 핵심을 담은 zettel 제목 생성
      let title = this.dag.title || "";
      try {
        const { llmText } = require("../core/llm");
        const stagesSummary = this.dag.stageHistory
          .map((s) => `${s.stage}(${s.status})`)
          .join(", ");
        const titlePrompt = `아래 forge 태스크 결과를 읽고, 이 작업이 무엇을 달성했는지 핵심을 담은 한 문장 제목을 작성하세요.
기술적 변경의 본질을 포착하되, 나중에 검색할 때 유용한 키워드를 포함하세요.
30-60자 내외, 한국어, 제목만 출력하세요.

## 태스크 정보
- ID: ${this.taskId}
- 기존 제목: ${title || "(없음)"}
- 파이프라인: ${this.dag.pipeline}
- 스테이지: ${stagesSummary}

## 요약
${summary.slice(0, 2000)}`;
        const { text } = await llmText(titlePrompt, {
          model: "haiku",
          hardTimeoutMs: 30000,
        });
        ...
        this.emit("agent:output", {
          stage: "hivemind",
          chunk: `[forge] zettel title generation failed, using fallback: ${e.message}`,
        });
      }
      // fallback: summary 첫 줄 또는 taskId
      if (!title) {
        const firstLine = summary.split("\n").find((l) => l.trim());
        title = firstLine ? firstLine.replace(/^#+\s*/, "").trim() : this.taskId;
      }
