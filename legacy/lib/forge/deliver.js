...
async function run({ taskId, dag, project, autoApprove, onLog = () => {} } = {}) {
  ...
  if (!diffSummary) {
    try {
      ...
      onLog(
        "[deliver] using fallback: design.md for summary (no diff available)",
      );
    } catch (e) {
      if (e.code !== "ENOENT")
        onLog(`[deliver] loadArtifact error (design.md): ${e.message}`);
      try {
        diffSummary = await loadArtifact(taskId, "spec.md");
        onLog(
          "[deliver] using fallback: spec.md for summary (no diff or design available)",
        );
      } catch (e2) {
        if (e2.code !== "ENOENT")
          onLog(`[deliver] loadArtifact error (spec.md): ${e2.message}`);
        onLog(
          "[deliver] warning: no diff, design, or spec artifact available for summary",
        );
        diffSummary = "";
      }
    }
  ...
  if (autoApprove && dag.warnings.length === 0) {
    // autoApprove + 경고 없음: 자동 머지
      ...
      try {
        ...
        if (mqEnabled) {
          try {
            mqManager.enqueue(taskId, project, 0);
            onLog(
              "[deliver] auto-merge failed, enqueued to merge queue for retry",
            );
            dag.status = "merge_queued";
          } catch (enqueueError) {
            onLog(
              `[deliver] merge queue enqueue also failed: ${enqueueError.message}`,
            );
            dag.status = "review";
          }
        } else {
