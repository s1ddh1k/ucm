...
function normalizeProjects(meta) {
  ...
  function sanitizeProjectName(name, resolvedPath) {
    ...
    // Prevent path traversal in user-provided display names by keeping basename only.
    const posixLike = trimmed.replace(/\\/g, "/");
    const base = path.posix.basename(posixLike);
  ...
  function toProject(entry, index) {
    ...
    const project = {
      path: resolvedPath,
      name: sanitizeProjectName(inputName, resolvedPath),
