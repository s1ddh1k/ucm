...
import { deriveLifecycleKindFromDecision } from "./runtime-policy";
import {
  appendDeliverableRevision,
  appendHandoff,
...
export function decideFromContext(
  context: ConductorDecisionContext,
): ConductorDecision {
  const { event, latestArtifactType, hasDeliverable } = context;

  if (!hasDeliverable) {
    return {
      decision: "observe",
      summary: "Autopilot skipped conductor output because no deliverable is available yet.",
    };
  }
  ...
  if (event.kind === "completed" && event.metadata?.source === "approval") {
    return {
      decision: "observe",
      summary:
        "Approved work should hand off to the next packaging lane instead of reopening review on the same run.",
    };
  }
  ...
  if (event.kind === "needs_review" || event.kind === "completed") {
    return {
      decision: "prepare_revision_and_request_review",
      summary:
        "Review-ready work should arrive as a fresh deliverable revision in the reviewer inbox.",
      revisionSummary:
    ...
    return {
      ...
      summary:
        latestArtifactType === "diff"
          ...
          : "New artifacts should roll into the current deliverable without interrupting the loop.",
      revisionSummary:
        latestArtifactType === "diff"
          ? "Autopilot rolled the latest code diff into a fresh deliverable revision for passive review."
          : "Autopilot rolled the latest artifacts into a fresh deliverable revision for passive review.",
    };
  }
...
export function applyConductorDecision(
  state: RuntimeState,
  run: RunDetail,
  ...
  const revision = appendDeliverableRevision(run, deliverable.id, {
