...
피드백과 함께 반려한다. 이후 `ucm resume <id>`로 피드백을 반영한 재작업을 실행한다.
...
```bash
# 상태 조회
curl -s http://localhost:17172/api/autopilot/status

# 세션 시작
curl -s -X POST http://localhost:17172/api/autopilot/start \
  -H 'content-type: application/json' \
  -d '{"project":"/path/to/project"}'

# Directive 추가
curl -s -X POST http://localhost:17172/api/autopilot/directive/add \
  -H 'content-type: application/json' \
  -d '{"sessionId":"ap_xxxx","text":"로그인에 2FA 추가"}'
```

`ucm` CLI에는 현재 `ucm socket ...` passthrough 명령이 없으므로, Autopilot 제어는 대시보드 또는 위 API를 사용한다.
