...
ucm logs forge-20260219-a3f2 --follow   # 실시간 follow
ucm logs forge-20260219-a3f2 --watch    # --follow 별칭(호환)
```
...
```bash
ucm submit <file.md>                               # 태스크 파일 등록 (pending)
ucm submit --project <dir> --title "..." < body.md # stdin 본문으로 등록
ucm cancel <task-id>                                # pending/running 취소
ucm retry <task-id>                                 # failed 태스크 재시도
ucm delete <task-id> [--force]                      # done/failed 태스크 삭제
ucm priority <task-id> <N>                          # 큐 우선순위 조정
ucm gate approve <task-id>                          # Stage Gate 승인
ucm gate reject <task-id> --feedback "..."          # Stage Gate 반려
```

### `ucm analyze` / `ucm research` — 프로젝트 분석/리서치

```bash
ucm analyze --project ~/git/myapp
ucm research --project ~/git/myapp
```

두 명령 모두 Observer 기반으로 개선 제안을 생성하며, 결과는 `ucm proposals`와 대시보드 Proposals 화면에서 확인한다.

### `ucm proposals` / `ucm proposal ...` — 제안서 운영

```bash
ucm proposals --status proposed
ucm proposal approve <id>
ucm proposal reject <id> --force
ucm proposal up <id> / ucm proposal down <id>
ucm proposal eval <id>
ucm proposal score <id>
ucm proposal score <id> --impact 4 --urgency 5 --uncertainty 2 --executionCost 3 --cwFitness 4
ucm proposal weights [--profile stabilization]
ucm proposal clusters [--refresh]
ucm proposal cluster-merge <id1> <id2> [<id3>...]
ucm proposal cluster-split <id>
ucm proposal conflicts <id>
ucm proposal discard <id> --reason "..."
ucm proposal history [--limit 20]
ucm proposal readiness <id>
```
...
```bash
ucm mode
ucm mode set stabilization --reason "회귀 안정화 주간"
ucm mode set big_bet --reason "대규모 투자 전환"
ucm mode criteria
```

### `ucm merge-queue` — 머지 큐 운영
...
**왼쪽 패널 — 작업 목록:**
- 상태/프로젝트 필터 + 검색 + 정렬(Newest/Priority/Title) 지원
...
**왼쪽 패널:**
- 상태/카테고리/위험도/프로젝트 필터 지원, 기본 정렬은 priority 내림차순
