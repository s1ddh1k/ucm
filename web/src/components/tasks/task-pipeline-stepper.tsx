import { Check, Play, Circle, Pause } from "lucide-react";
...
export function TaskPipelineStepper({ pipeline, currentStage, state, stageGate }: TaskPipelineStepperProps) {
  ...
  return (
    <div className="flex items-center gap-1 overflow-x-auto py-2">
      {stages.map((stage, i) => {
        ...
        const isDone = state === "done" || state === "review"
          ? true
          : currentIndex > i;
        const isCurrent = currentIndex === i && state === "running" && !isGateWaiting;
        const isPending = !isDone && !isCurrent && !isGateWaiting;

        return (
          <div key={stage} className="flex items-center gap-1">
            {i > 0 && (
              <div className={cn("h-px w-4", isDone ? "bg-emerald-400" : isGateWaiting ? "bg-amber-400" : "bg-muted")} />
            )}
