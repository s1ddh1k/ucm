...
function testReqCliKeepsGapReportForFollowupQna() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "req-gap-followup-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "  if (!feedbackFile || !fs.existsSync(path.resolve(feedbackFile))) {",
      "    console.error('missing feedback file on follow-up round');",
      "    process.exit(2);",
      "  }",
      "  const feedbackText = fs.readFileSync(path.resolve(feedbackFile), 'utf-8');",
      "  if (!feedbackText.includes('# Gap Report')) {",
      "    console.error('invalid feedback file content');",
      "    process.exit(3);",
      "  }",
      "  fs.writeFileSync(path.join(outputDir, 'followup-feedback-ok'), 'ok');",
      "}",
...
async function testRefinementFinalizeSurvivesFinalizedBroadcastFailure() {
  const events = [];
  const state = { stats: { totalSpawns: 0 } };
  const answerPlan = [];
  for (const [area, count] of Object.entries(REFINEMENT_GREENFIELD)) {
  ...
  ucmdRefinement.setDeps({
    ...
    broadcastWs: (event, data) => {
      events.push({ event, data });
      if (event === "refinement:finalized") {
        throw new Error("finalized event delivery failed intentionally");
      }
    },
    submitTask: async () => {
      submitCalls += 1;
      return { id: "finalize-broadcast-failure-task" };
    },
    spawnAgent: async () => ({
      status: "done",
      stdout: JSON.stringify({
...
async function testRefinementCleansSessionAfterRepeatedInvalidQuestionArea() {
  const events = [];
  const state = { stats: { totalSpawns: 0 } };
  let spawnCalls = 0;
  let sessionId = null;
    ...
    spawnAgent: async () => {
      ...
      return {
        ...
        stdout: JSON.stringify({
          ...
          question: "invalid area question",
          options: [{ label: "옵션", reason: "이유" }],
          area: "알 수 없는 영역",
        }),
      };
  ...
  try {
    const started = await ucmdRefinement.startRefinement({
      title: "invalid question area cleanup",
      description: "session should be cleaned when area is out of expected set",
      mode: "interactive",
    });
    sessionId = started.sessionId;
...
async function testRefinementCleansSessionAfterRepeatedJsonParseFailure() {
  ...
  ucmdRefinement.setDeps({
    ...
    spawnAgent: async () => {
      spawnCalls += 1;
      return { status: "done", stdout: "not valid json" };
    },
  });

  try {
    const started = await ucmdRefinement.startRefinement({
      title: "json parse hard failure cleanup",
      description: "session should be cleaned when parse retries are exhausted",
      mode: "interactive",
    });
    sessionId = started.sessionId;
...
async function main() {
  ...
  await testRefinementCleansSessionAfterRepeatedInvalidQuestionArea();
