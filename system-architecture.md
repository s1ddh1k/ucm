...
### 1.6 Cache-Local Execution

자기개선형 UCM은 durable state만으로 충분하지 않다. 토큰 비용과 지연을 통제하려면 context locality도 1급 원칙이어야 한다.

- `durable runtime != full prompt replay`
- hot context는 가능하면 같은 provider session 안에 유지하고, cold context는 artifact store에 남긴다
- run 입력은 전문 재주입보다 `stable prefix + artifact reference + delta instruction`을 우선한다
- 역할 분리는 유지하되, 같은 repo 맥락을 반복 주입하는 과도한 micro-agent fan-out은 피한다
- transcript 전체 전달은 fallback이고, 기본 handoff 단위는 artifact ID, evidence summary, decision delta여야 한다
- provider가 지원하면 prompt prefix stability와 session affinity를 활용할 수 있게 prompt 조립과 scheduler가 함께 설계되어야 한다
...
- 같은 mission/phase에서 session affinity를 유지할지, 새 session으로 분기할지 결정
- prompt prefix stability와 cache locality를 깨지 않도록 run 배치를 조정
...
- session reuse 가능성은 provider seat의 속성으로 관리한다
- 같은 phase에서 반복 실행되는 run은 가능한 한 동일 provider/surface와 stable prefix를 유지한다
- 캐시 이득보다 독립 검증이 중요한 단계에서는 의도적으로 session affinity를 끊을 수 있어야 한다
...
- handoff 기본 단위는 transcript dump가 아니라 artifact reference와 delta context다
- 같은 run family는 가능한 한 동일한 context prefix를 공유하도록 prompt를 조립한다
- full repository summary 재주입은 기본값이 아니라 cache miss fallback이어야 한다
