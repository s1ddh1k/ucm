function deriveMissionStatus(currentStatus, eventKind) {
  if (
...
function deriveLifecycleKindFromDecision(decision) {
...
function phaseRequiresRole(phaseObjective, role) {
...
function artifactRequiresRole(artifactType, role) {
  if (role === "verification") {
...
function blockerRequiresRole(requestedInput, role) {
  if (role !== "research") return false;
  return requestedInput === "fixture_path" || requestedInput === "external_context";
}

function deriveLifecycleTransitions(agents, missionDetail, run, event) {
  const transitions = [];
...
module.exports = {
  deriveMissionStatus,
  deriveLifecycleKindFromDecision,
