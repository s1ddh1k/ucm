...
function timestampLabelFromRevision(revision) {
  return revision === 1 ? "just now" : `${revision - 1} version${revision > 2 ? "s" : ""} later`;
}

function createMissionInState(state, input) {
      ...
      handoffs: [
        {
          ...
          releaseRevisionId: `del-${mission.id}-r1`,
          channel: "inbox",
          createdAtLabel: "just now",
          status: "active",
        },
...
function generateReleaseRevisionInState(state, input) {
  ...
  const release = located.run.releases.find(
  ...
  const revisionId = `${release.id}-r${nextRevisionNumber}`;
  const nextRevision = {
    id: revisionId,
  ...
  located.run.decisions = [
    ...located.run.decisions,
    {
      id: `d-release-${Date.now()}`,
      category: "approval",
      summary: `Prepared ${release.title} version v${nextRevision.revision}.`,
      rationale: "Releases evolve append-only so reviewers can compare versions over time.",
    },
  ];
  appendRunEvent(state, located.run.id, {
    kind: "artifact_created",
    agentId: located.run.agentId,
    summary: `Manual version v${nextRevision.revision} was generated for ${release.title}.`,
    createdAtLabel: "just now",
  });
...
function handoffReleaseInState(state, input) {
  const located = findRun(state, input.runId);
  if (!located) {
    return null;
  }

  const revision = located.run.releases
    .flatMap((release) => release.revisions)
    .find((item) => item.id === input.releaseRevisionId);
  if (!revision) {
    return hydrateRunDetail(state, located.run);
  }
  ...
  located.run.timeline = [
    ...located.run.timeline,
    {
      ...
      kind: "completed",
      summary: `Delivered release ${revision.id} to ${input.target?.trim() || "human reviewer"}.`,
      timestampLabel: "just now",
    },
  ];
  appendRunEvent(state, located.run.id, {
    ...
    agentId: located.run.agentId,
    summary: `Latest release version was handed off to ${input.target?.trim() || "human reviewer"}.`,
    createdAtLabel: "just now",
  });
...
function approveReleaseRevisionInState(state, input) {
  const located = findRun(state, input.runId);
  if (!located) {
    return null;
  }

  let approvedRevisionId = null;
  for (const release of located.run.releases) {
    release.revisions = release.revisions.map((revision) => {
      if (revision.id === input.releaseRevisionId) {
  ...
  located.run.handoffs = located.run.handoffs.map((handoff) => {
    if (handoff.releaseRevisionId === approvedRevisionId) {
  ...
  located.run.timeline = [
    ...located.run.timeline,
    {
      ...
      kind: "completed",
      summary: `Human observer approved release ${approvedRevisionId}.`,
      timestampLabel: "just now",
    },
  ];
  located.run.decisions = [
    ...located.run.decisions,
    {
      ...
      category: "approval",
      summary: `Approved release ${approvedRevisionId}.`,
...
function submitSteeringInState(state, input) {
