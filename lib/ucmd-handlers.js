  ...
  return enqueueTaskMove(taskId, async () => {
    ...
    try {
      ...
      if (error && error.code !== "ENOENT") {
        // Keep state authoritative in one directory only when source cleanup fails.
...
async function handleSetConfig(params) {
  ...
  const operation = _configWriteLock.catch(() => {}).then(async () => {
  ...
  _configWriteLock = operation.catch(() => {});
  return operation;
}
