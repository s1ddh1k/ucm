const { readFile, writeFile, readdir, unlink, rm, rename } = require("fs/promises");
...
const _moveTaskLocks = new Set();

async function moveTask(taskId, from, to, metaUpdates) {
  // Guard against concurrent moves on the same task
  if (_moveTaskLocks.has(taskId)) return;
  _moveTaskLocks.add(taskId);
  try {
...
async function handleList(params) {
  ...
  const priorityFilter = params.minPriority != null ? Number(params.minPriority) : null;
  ...
  for (const state of states) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Load DAG summary for running/review tasks
...
async function handleReject(params) {
  ...
  if (feedback) {
    ...
    await moveTask(taskId, "review", "running", { feedback, completedAt: null });
