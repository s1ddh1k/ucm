...
async function recoverRunningTasks() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      deps.log(`recoverRunningTasks: readdir failed: ${e.message}`);
    return 0;
  }
  ...
  for (const file of files) {
    ...
    if (!deps.inflightTasks.has(taskId)) {
      try {
        ...
        deps.log(
          `recoverRunningTasks: failed to check suspended state for ${taskId}: ${e.message}`,
        );
      }
      try {
        ...
        deps.log(
          `recovered orphaned task: ${taskId} (running → pending, worktrees cleaned)`,
        );
        recovered++;
      } catch (e) {
