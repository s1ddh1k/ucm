...
function resolveAutomation(projectPath) {
  const config = deps.config?.() || {};
  const automation = config.automation || {};
  const global = {
    autoExecute: !!automation.autoExecute,
    autoApprove: !!automation.autoApprove,
    autoPropose: !!automation.autoPropose,
    autoConvert: !!automation.autoConvert,
  };
  if (!projectPath) return global;
  const projectKey = path.basename(projectPath);
  const overrides = automation.projects?.[projectKey];
  if (!overrides) return global;
  return {
    autoExecute: overrides.autoExecute ?? global.autoExecute,
    autoApprove: overrides.autoApprove ?? global.autoApprove,
    autoPropose: overrides.autoPropose ?? global.autoPropose,
    autoConvert: overrides.autoConvert ?? global.autoConvert,
  };
}

async function tryAutoApprove(taskId) {
  try {
    const task = await loadTask(taskId);
    if (!task || task.state !== "review") return;
    const projects = normalizeProjects(task);
    const projectPath = projects[0]?.path || null;
    const toggles = resolveAutomation(projectPath);
    if (!toggles.autoApprove) return;
    deps.log(`[autoApprove] auto-approving task ${taskId}`);
    await handleApprove({ taskId });
  } catch (e) {
    deps.log(`[autoApprove] failed for ${taskId}: ${e.message}`);
  }
}
...
function isRetryableIoError(error) {
  return !!(error?.code && RETRYABLE_IO_CODES.has(error.code));
}
...
function withIoContext(operation, error, context = {}) {
  const details = Object.entries(context)
    .filter(([, value]) => value !== undefined && value !== null && value !== "")
    .map(([key, value]) => `${key}=${value}`)
    .join(" ");
  const suffix = details ? ` (${details})` : "";
  const wrapped = new Error(`${operation} failed${suffix}: ${error.message}`);
  wrapped.code = error?.code;
  wrapped.cause = error;
  wrapped.retryable = isRetryableIoError(error);
...
async function atomicWriteFile(filePath, content, context = {}) {
  ...
  try {
    ...
    const wrapped = withIoContext(context.operation || "atomic write", error, {
      ...context,
      filePath,
      tmpPath,
    });
...
async function loadDagSummary(taskId) {
  try {
    ...
    if (error?.code !== "ENOENT") {
      deps.log(
        `[loadDagSummary] task=${taskId} stage=dag-summary retryable=${error?.retryable === true} ${error.message}`,
      );
    }
    return null;
  }
...
async function submitTask(title, body, options = {}) {
  ...
  await atomicWriteFile(taskPath, taskContent, {
    operation: "submitTask write",
    taskId,
    stage: "submit-task",
  });
  ...
  const projectPath = meta.project || null;
  const toggles = resolveAutomation(projectPath);
  if (toggles.autoExecute) {
    handleStart({ taskId }).catch((e) =>
      deps.log(`[autoExecute] failed for ${taskId}: ${e.message}`),
    );
  }
...
async function cleanupDuplicateTaskStateFiles(taskId, keepState) {
  const cleanupTargets = TASK_STATES.filter((state) => state !== keepState);
  for (const state of cleanupTargets) {
    const stalePath = path.join(TASKS_DIR, state, `${taskId}.md`);
    try {
      await unlink(stalePath);
      deps.log(
        `[${taskId}] moveTask: removed stale duplicate task file from ${state}`,
      );
    } catch (e) {
      if (e?.code !== "ENOENT") {
        deps.log(
          `[${taskId}] moveTask: failed to clean duplicate ${stalePath}: ${e.message}`,
        );
      }
    }
  }
  ...
  return enqueueTaskFileOp(
    ...
    async () => {
      ...
      try {
        ...
        throw withIoContext("moveTask read", error, {
          taskId,
          from,
          to,
          stage: "task-state-transition",
          filePath: srcPath,
        });
      }
      let parsed;
      try {
        ...
        throw withIoContext("moveTask parse", error, {
          taskId,
          from,
          to,
          stage: "task-state-transition",
      ...
      if (to === "review" || to === "done" || to === "failed")
      ...
      if ((to === "done" || to === "failed") && meta.id && !meta.tokenUsage) {
        try {
          ...
          if (dagSummary) {
            ...
            if (dagSummary.currentStage)
              meta.currentStage = dagSummary.currentStage;
          }
        } catch {
      ...
      const serializedTask = serializeTaskFile(meta, body);
      if (from === to) {
        await atomicWriteFile(srcPath, serializedTask, {
          operation: "moveTask in-place update",
          taskId,
          from,
          to,
          stage: "task-state-transition",
        ...
        await atomicWriteFile(dstPath, serializedTask, {
          operation: "moveTask destination update",
          taskId,
          from,
          to,
          stage: "task-state-transition",
        ...
        try {
          ...
          if (error && error.code !== "ENOENT") {
            ...
            deps.log(
              `[${taskId}] moveTask: failed to remove source ${from}/${taskId}.md (path=${srcPath}, stage=task-state-transition): ${error.message} (orphaned)`,
            );
          }
        }
      }
      await cleanupDuplicateTaskStateFiles(taskId, to);
      deps.broadcastWs("task:updated", { taskId, state: to });
    },
    {
      log: deps.log,
      label: `moveTask(${from}->${to})`,
    },
  );
}

function compareQueuedTasks(a, b) {
  if ((b.priority || 0) !== (a.priority || 0))
    return (b.priority || 0) - (a.priority || 0);
  return (a.created || "").localeCompare(b.created || "");
}

function enqueuePendingTask(task, context = "queue") {
  if (!task?.id) {
    throw new Error(`${context}: missing task id`);
  }
  if (!Array.isArray(deps.taskQueue)) {
    throw new Error(`${context}: taskQueue not initialized`);
  }

  const taskId = task.id;
  if (deps.inflightTasks?.has?.(taskId)) {
    return { id: taskId, status: "running", queued: false };
  }
  if (deps.taskQueueIds?.has?.(taskId)) {
    return { id: taskId, status: "queued", queued: false };
  }
  ...
  deps.taskQueue.sort(compareQueuedTasks);

  if (typeof deps.wakeProcessLoop === "function") {
    deps.wakeProcessLoop();
  }

  return { id: taskId, status: "queued", queued: true };
}
...
async function scanPendingTasks() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      deps.log(`scanPendingTasks: readdir failed: ${e.message}`);
    return [];
  }
...
async function loadTask(taskId) {
  for (const state of TASK_STATES) {
    ...
    try {
      ...
      if (e && e.code !== "ENOENT") {
        deps.log(`loadTask: error reading ${taskPath}: ${e.message}`);
        throw e;
      }
    }
  }
...
async function recoverRunningTasks() {
  ...
  try {
    ...
    if (e && e.code !== "ENOENT")
      deps.log(
        `recoverRunningTasks: readdir failed (${runningDir}): ${e.message}`,
      );
    return 0;
  }
  ...
  for (const file of files) {
    ...
    if (!deps.inflightTasks.has(taskId)) {
      const taskPath = path.join(runningDir, file);
      let taskMeta = null;
      try {
        const content = await readFile(taskPath, "utf-8");
        ...
        // merge-queue tasks: preserve, and re-enqueue when persisted queue state is missing
        if (
          meta.mergeQueue === "queued" ||
          meta.mergeQueue === "merging" ||
          meta.mergeQueue === "rebasing"
        ) {
          const queueProject = normalizeProjects(meta)[0]?.path;
          const mqManager = deps.mergeQueueManager;
          if (
            mqManager &&
            queueProject &&
            typeof mqManager.getStatus === "function" &&
            typeof mqManager.enqueue === "function"
          ) {
            const queueStatus = mqManager.getStatus(queueProject);
            const queueEntries = Array.isArray(queueStatus?.entries)
              ? queueStatus.entries
              : [];
            const queueHasTask =
              queueStatus?.current?.taskId === taskId ||
              queueEntries.some((entry) => entry?.taskId === taskId);
            if (!queueHasTask) {
              try {
                mqManager.enqueue(taskId, queueProject, meta.priority || 0);
                deps.log(
                  `recoverRunningTasks: re-enqueued merge-queue task ${taskId} for ${queueProject} (mergeQueue=${meta.mergeQueue})`,
                );
              } catch (queueError) {
                deps.log(
                  `recoverRunningTasks: failed to re-enqueue merge-queue task ${taskId} for ${queueProject}: ${queueError.message}`,
                );
              }
            }
          }
          ...
          deps.log(
            `preserved merge-queue task: ${taskId} (mergeQueue: ${meta.mergeQueue})`,
          );
          continue;
        }
        ...
        if (meta.suspended) {
          ...
          deps.log(
            `preserved suspended task: ${taskId} (stage: ${meta.suspendedStage || "unknown"})`,
          );
          continue;
        }
      } catch (e) {
        deps.log(
          `recoverRunningTasks: failed to inspect ${taskPath} for task ${taskId}: ${e.message}`,
        );
      }
      try {
        ...
        if (!taskMeta) {
          const content = await readFile(taskPath, "utf-8");
          taskMeta = parseTaskFile(content).meta;
        }
        const projects = normalizeProjects(taskMeta);
        await removeWorktrees(taskId, projects);
        await moveTask(taskId, "running", "pending");
        const pendingTask = await loadTask(taskId);
        if (pendingTask?.state === "pending") {
          enqueuePendingTask(pendingTask, "recoverRunningTasks");
        }
        deps.log(
          `recoverRunningTasks: recovered ${taskId} (running/${file} -> pending/${file}, stage: ${taskMeta?.currentStage || "unknown"})`,
        );
        recovered++;
      } catch (e) {
        deps.log(
          `recoverRunningTasks: failed to recover ${taskId} from ${taskPath}: ${e.message}`,
        );
      }
    }
  }
...
async function handleSubmit(params) {
  const { title, body, project, projects, priority, pipeline, taskFile } =
    params;

  if (taskFile) {
    const { meta, body: fileBody } = parseTaskFile(taskFile);
    return submitTask(meta.title || "untitled", fileBody, {
...
async function handleStart(params) {
  ...
  if (task.state !== "pending")
    throw new Error(`task is not pending: ${task.state}`);

  const queued = enqueuePendingTask(task, "handleStart");
  if (!queued.queued) {
    return { id: taskId, status: queued.status };
  }
...
async function handleList(params) {
  ...
  for (const state of states) {
    ...
    try {
      ...
      if (e && e.code !== "ENOENT")
        deps.log(`handleList: readdir failed for ${state}: ${e.message}`);
      continue;
    }
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (projectFilter) {
          ...
          if (!taskProjects.some((p) => path.resolve(p.path) === projectFilter))
            continue;
        }
        if (
          titleFilter &&
          !(meta.title || "").toLowerCase().includes(titleFilter)
        )
          continue;
        if (pipelineFilter && meta.pipeline !== pipelineFilter) continue;
        if (priorityFilter != null && (meta.priority || 0) < priorityFilter)
          continue;
...
async function countTasksByState() {
  ...
  for (const state of TASK_STATES) {
    ...
    try {
      ...
      if (e && e.code !== "ENOENT")
        deps.log(
          `countTasksByState: readdir failed for ${state}: ${e.message}`,
        );
      counts[state] = 0;
    }
  }
...
async function handleApprove(params) {
  ...
  if (task.state !== "review")
    throw new Error(`task is not in review state: ${task.state}`);
  ...
  if (selfConfig.enabled && projects.some((p) => isSelfTarget(p.path))) {
    if (selfConfig.requirePassingTests !== false) {
      ...
      deps.log(
        `[${taskId}] self-modification safety gate passed (backup: ${gate.backupBranch})`,
      );
    }
  }

  const mqManager = deps.mergeQueueManager;
  const mqEnabled = mqManager?.isEnabled();

  if (mqEnabled && mqManager.isBusy(projects[0]?.path)) {
    // merge queue busy → enqueue to serialize merges
    await moveTask(taskId, "review", "running", { mergeQueue: "queued" });

    // track in daemonState so daemon restart doesn't discard the task
    if (!daemonState.activeTasks.includes(taskId)) {
      daemonState.activeTasks.push(taskId);
    }
    deps.markStateDirty();

    mqManager.enqueue(taskId, projects[0].path, task.priority || 0);
    deps.log(`task approved (enqueued to merge queue): ${taskId}`);

    if (score !== undefined) {
      await updateMemory(taskId, { metrics: { score } });
    }

    evaluateProposal(taskId).catch((e) =>
      deps.log(`[evaluate] ${taskId}: ${e.message}`),
    );
    return { id: taskId, status: "merging" };
  }
  ...
  if (daemonState.suspendedTasks) {
    daemonState.suspendedTasks = daemonState.suspendedTasks.filter(
      (t) => t !== taskId,
    );
  }
  deps.markStateDirty();
...
async function handleReject(params) {
  ...
  if (feedback) {
    ...
    wireEvents(fp, (event, data) => {
      ...
      if (event === "stage:gate") {
        const op = deps.updateTaskMeta?.(taskId, { stageGate: data.stage });
        if (op && typeof op.catch === "function") {
          op.catch((e) =>
            deps.log(
              `[${taskId}] updateTaskMeta stageGate=${data.stage} failed: ${e.message}`,
            ),
          );
        }
      }
      if (event === "stage:gate_resolved") {
        const op = deps.updateTaskMeta?.(taskId, { stageGate: null });
        if (op && typeof op.catch === "function") {
          op.catch((e) =>
            deps.log(
              `[${taskId}] updateTaskMeta stageGate=null failed: ${e.message}`,
            ),
          );
        }
      }
    ...
    fp.run()
      .then(async (dag) => {
        await deps.drainTaskMetaQueue?.(taskId);
      ...
      .catch(async (e) => {
        ...
        try {
          ...
          deps.log(
            `reject-feedback moveTask to failed also errored for ${taskId}: ${e2.message}`,
          );
        }
      })
      .finally(async () => {
        ...
        if (typeof deps.flushStateNow === "function") {
          await deps.flushStateNow();
        } else {
          deps.markStateDirty();
        }
      });
  ...
  if (Array.isArray(daemonState.activeTasks)) {
    ...
    trackingChanged =
      trackingChanged || nextActive.length !== daemonState.activeTasks.length;
    daemonState.activeTasks = nextActive;
  }
  if (Array.isArray(daemonState.suspendedTasks)) {
    const nextSuspended = daemonState.suspendedTasks.filter(
      (t) => t !== taskId,
    );
    trackingChanged =
      trackingChanged ||
      nextSuspended.length !== daemonState.suspendedTasks.length;
...
async function handleCancel(params) {
  ...
  if (fp) {
    try {
      ...
      deps.log(`abort failed for ${taskId}: ${e.message}`);
    }
    deps.activeForgePipelines.delete(taskId);
  }
...
async function handleRetry(params) {
  ...
  if (task.state !== "failed")
    throw new Error(`task is not failed: ${task.state}`);
...
async function handleDelete(params) {
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      deps.log(
        `delete: failed to remove task file for ${taskId}: ${e.message}`,
      );
  }
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      deps.log(`delete: failed to remove log file for ${taskId}: ${e.message}`);
  }
  try {
    await rm(path.join(LOGS_DIR, taskId), { recursive: true });
  } catch (e) {
    if (e.code !== "ENOENT")
      deps.log(`delete: failed to remove log dir for ${taskId}: ${e.message}`);
  }
  try {
    ...
    if (e.code !== "ENOENT")
      deps.log(
        `delete: failed to remove artifacts for ${taskId}: ${e.message}`,
      );
  }
...
async function handleLogs(params) {
  ...
  try {
    ...
    throw new Error(
      `failed to read logs for ${taskId}: ${detail}. Check file permissions for ${logPath} and run \`ucm status ${taskId}\` to verify task state.`,
    );
  }
}
...
async function handlePause() {
  ...
  try {
    ...
    deps.log(`flush error after pause: ${e.message}`);
  }
...
async function handleResume() {
  ...
  try {
    ...
    try {
      ...
      deps.log(`flush error after resume failure: ${fe.message}`);
    }
    deps.log(
      `daemon resume failed while requeueing suspended tasks: ${e.message}`,
    );
    deps.broadcastWs("daemon:status", { status: "paused" });
  ...
  try {
    ...
    deps.log(`flush error after resume: ${e.message}`);
  }
...
async function handleStats() {
  ...
  const counts = await countTasksByState();
  const llmProvider =
    config?.provider || process.env.UCM_PROVIDER || DEFAULT_CONFIG.provider;
...
async function handleSetConfig(params) {
  if (params.provider !== undefined) {
    if (
      ...
      !VALID_PROVIDERS.has(params.provider)
    ) {
      throw new Error(
        `invalid provider: ${params.provider} (valid: ${[...VALID_PROVIDERS].join(", ")})`,
      );
    }
  }
  ...
  const operation = _configWriteLock
    .catch((e) => {
      deps.log(`config write lock error (non-fatal): ${e.message}`);
    })
    .then(async () => {
      const current = deps.config();
      const merged = { ...current, ...params };
      await atomicWriteFile(CONFIG_PATH, `${JSON.stringify(merged, null, 2)}\n`, {
        operation: "handleSetConfig write",
        stage: "config-update",
      });
  ...
  _configWriteLock = operation.catch((e) => {
    deps.log(`config write error (non-fatal): ${e.message}`);
  });
  return operation;
}

async function handleMergeQueueStatus(params) {
  const mqManager = deps.mergeQueueManager;
  if (!mqManager) throw new Error("merge queue not available");
  return mqManager.getStatus(params.project);
}

async function handleMergeQueueRetry(params) {
  const { taskId } = params;
  if (!taskId) throw new Error("taskId required");
  const mqManager = deps.mergeQueueManager;
  if (!mqManager) throw new Error("merge queue not available");

  const retried = mqManager.retry(taskId);
  if (!retried) {
    // not in queue — try to re-enqueue from review
    const task = await loadTask(taskId);
    if (!task) throw new Error(`task not found: ${taskId}`);
    if (task.state !== "review")
      throw new Error(`task is not in review state: ${task.state}`);
    ...
    mqManager.enqueue(taskId, project, task.priority || 0);
  }

  deps.log(`merge queue retry: ${taskId}`);
  return { id: taskId, status: "queued" };
}

async function handleMergeQueueSkip(params) {
  const { taskId } = params;
  if (!taskId) throw new Error("taskId required");
  const mqManager = deps.mergeQueueManager;
  if (!mqManager) throw new Error("merge queue not available");

  const skipped = mqManager.skip(taskId);
  if (!skipped)
    throw new Error(
      `task ${taskId} not found in merge queue or currently processing`,
    );

  // move task back to review and clean up daemon tracking
  try {
    await moveTask(taskId, "running", "review", {
      mergeQueue: null,
    ...
    deps.log(`merge queue skip: moveTask failed for ${taskId}: ${e.message}`);
  }
  const daemonState = deps.daemonState();
  ...
  deps.log(`merge queue skip: ${taskId}`);
  return { id: taskId, status: "skipped" };
}
...
async function handleUpdatePriority(params) {
  ...
  await enqueueTaskFileOp(
    ...
    async () => {
      ...
      if (task.state !== "pending") {
        throw new Error(
          `can only change priority of pending tasks: ${task.state}`,
        );
      }
      ...
      await atomicWriteFile(taskPath, serializeTaskFile(meta, body), {
        operation: "handleUpdatePriority write",
        taskId,
        stage: "priority-update",
      });
    ...
    { log: deps.log, label: "handleUpdatePriority" },
  );
