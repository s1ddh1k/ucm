  ...
  for (const file of files) {
    ...
    if (!deps.inflightTasks.has(taskId)) {
      try {
        ...
        if (meta.mergeQueue === "queued" || meta.mergeQueue === "merging" || meta.mergeQueue === "rebasing") {
          if (!daemonState.activeTasks.includes(taskId)) {
            daemonState.activeTasks.push(taskId);
            trackingChanged = true;
          }
          deps.log(`preserved merge-queue task: ${taskId} (mergeQueue: ${meta.mergeQueue})`);
          continue;
        }
...
async function handlePause() {
  ...
  try { await deps.flushStateNow(); } catch (e) { deps.log(`flush error after pause: ${e.message}`); }
...
async function handleResume() {
  ...
  try {
    ...
    try { await deps.flushStateNow(); } catch (fe) { deps.log(`flush error after resume failure: ${fe.message}`); }
  ...
  try { await deps.flushStateNow(); } catch (e) { deps.log(`flush error after resume: ${e.message}`); }
...
async function handleMergeQueueSkip(params) {
  ...
  if (!skipped) throw new Error(`task ${taskId} not found in merge queue or currently processing`);

  // move task back to review and clean up daemon tracking
  try {
    await moveTask(taskId, "running", "review", { mergeQueue: null, mergeRebaseCount: null });
  } catch (e) {
