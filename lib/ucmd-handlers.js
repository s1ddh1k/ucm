...
function enqueueTaskMove(taskId, operation) {
  ...
  const current = previous
    .catch((e) => {
      deps.log(
        `enqueueTaskMove: prior operation for ${taskId} failed: ${e.message}`,
      );
    })
    .then(operation);
  ...
  current
    ...
    .catch((e) => {
      deps.log(`enqueueTaskMove: cleanup error for ${taskId}: ${e.message}`);
    });
  return current;
}
  ...
  return enqueueTaskMove(taskId, async () => {
    ...
    try {
      ...
      if (error && error.code !== "ENOENT") {
        ...
        deps.log(
          `moveTask: failed to remove source ${from}/${taskId}.md: ${error.message} (orphaned)`,
        );
      }
    }
...
async function handleUpdatePriority(params) {
  ...
  if (!task) throw new Error(`task not found: ${taskId}`);
  if (task.state !== "pending")
    throw new Error(`can only change priority of pending tasks: ${task.state}`);
