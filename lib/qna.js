...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--feedback-file": opts.feedbackFile = args[++i]; break;
...
async function validate(opts) {
  ...
  if (opts.feedback && opts.feedbackFile) errors.push("--feedback 과 --feedback-file 동시 사용 불가");
  if (opts.template) {
  ...
  if (opts.feedbackFile) {
    try { await access(path.resolve(opts.feedbackFile)); }
    catch { errors.push(`feedback 파일 없음: ${opts.feedbackFile}`); }
  }
...
async function main() {
  ...
  await appendLog(logPath, { type: "start", timestamp: new Date().toISOString(), template: opts.template || null, project: opts.project || null, feedback, resumeDecisions: decisions.length });
  ...
  const feedbackStartDecisionsCount = (typeof feedback === "string" && feedback.trim().length > 0)
    ? decisions.length
    : null;
  const expectedAreas = new Set(Object.keys(isBrownfield ? EXPECTED_BROWNFIELD : EXPECTED_GREENFIELD));
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    const prompt = buildQuestionPrompt(template, decisions, feedback, { isResume: !!opts.resume, isBrownfield, coverage, repoContext });
    ...
    const responseArea = typeof response.area === "string" ? response.area.trim() : "";
    if (!responseArea || !expectedAreas.has(responseArea)) {
      process.stderr.write(`허용되지 않은 영역 응답 감지: ${response.area || "(없음)"} → 재시도...\n`);
