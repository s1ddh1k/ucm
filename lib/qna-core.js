...
const QUESTION_SIGNAL_TOKEN_ALIASES = new Map([
  ["db", "데이터베이스"],
  ["database", "데이터베이스"],
]);
...
function normalizeQuestionToken(token) {
  ...
  normalized = normalized.replace(
    /(?:인가요|인가|입니까|일까요|할까요|나요|까요|요)$/g,
    "",
  );
  normalized = normalized.replace(
    /(?:으로|로|은|는|이|가|을|를|과|와|도|만|에|의)$/g,
    "",
  );
  if (!normalized) return "";
  return QUESTION_SIGNAL_TOKEN_ALIASES.get(normalized) || normalized;
}
...
function isInformativeCoverageSignal(signal) {
  ...
  if (GENERIC_NON_ACTIONABLE_SIGNAL_RE.some((re) => re.test(signal)))
    return false;
  return true;
}
...
function computeCoverage(decisions, expectedOrBrownfield) {
  const expected =
    typeof expectedOrBrownfield === "object"
  ...
  for (const [area, count] of Object.entries(expected)) {
    ...
    for (const d of decisions) {
      ...
      if (isInformativeCoverageSignal(signal)) {
        const questionKey = questionSignal || signal;
        latestAnswerByQuestion.set(questionKey, signal);
        continue;
      }

      // Same question answered with "모름/TBD" should invalidate prior coverage
      // until it is answered informatively again.
      if (!questionSignal) continue;
      latestAnswerByQuestion.delete(questionSignal);
    }
    const seenQuestionSignals = new Set(latestAnswerByQuestion.keys());
    const seenAnswerSignals = new Set(latestAnswerByQuestion.values());
...
function collectUnresolvedContradictions(decisions) {
  ...
  for (const d of decisions) {
    const areaSignal = normalizeCoverageSignal(d?.area);
    const questionSignal = normalizeQuestionSignal(d?.question);
    if (!areaSignal || !questionSignal) continue;

    const answerSignal = normalizeCoverageSignal(
      (d && (d.requirement || d.answer)) || "",
    );
    ...
    if (!answersByQuestion.has(key)) {
      ...
      answersByQuestion.set(key, {
        ...
        hasInformativeAnswer: false,
        hasTrailingUncertainty: false,
        lastAnswerSignal: "",
      });
    }

    const entry = answersByQuestion.get(key);
    if (!isInformativeCoverageSignal(answerSignal)) {
      if (entry.hasInformativeAnswer) entry.hasTrailingUncertainty = true;
      continue;
    }
    ...
    entry.answerCounts.set(
      answerSignal,
      (entry.answerCounts.get(answerSignal) || 0) + 1,
    );
    entry.hasInformativeAnswer = true;
    entry.hasTrailingUncertainty = false;
    entry.lastAnswerSignal = answerSignal;
  }

  const contradictions = [];
  for (const entry of answersByQuestion.values()) {
    if (entry.hasTrailingUncertainty) {
      const answers = Array.from(entry.answers.values());
      answers.push("미정/불확실");
      contradictions.push({
        area: entry.area,
...
function shouldSkipDuplicateQuestion(decisions, candidate) {
  ...
  const areaSignal = normalizeCoverageSignal(candidate?.area);
  const questionSignal = normalizeQuestionSignal(candidate?.question);
  if (!areaSignal || !questionSignal) return false;

  let alreadyAsked = false;
  let hasInformativeAnswer = false;
  for (const decision of decisions) {
    if (normalizeCoverageSignal(decision?.area) !== areaSignal) continue;
    if (normalizeQuestionSignal(decision?.question) !== questionSignal)
      continue;
    alreadyAsked = true;
    const answerSignal = normalizeCoverageSignal(
  ...
  const requiresResolution = contradictions.some(
    (conflict) =>
      normalizeCoverageSignal(conflict?.area) === areaSignal &&
      normalizeQuestionSignal(conflict?.question) === questionSignal,
  );
...
function buildMeaningfulDecisionSignal(decision) {
  const areaSignal = normalizeCoverageSignal(decision?.area);
  const questionSignal = normalizeQuestionSignal(decision?.question);
  const answerSignal = normalizeCoverageSignal(
    decision && (decision.requirement || decision.answer),
  );
...
function areStructuredGapsAddressed(feedback, meaningfulFollowups) {
  ...
  if (!Array.isArray(meaningfulFollowups) || meaningfulFollowups.length === 0)
    return true;

  return gaps.every((gap) => {
    const keywords = extractGapKeywords(gap);
    return meaningfulFollowups.some((decision) =>
      followupAddressesGap(decision, keywords),
    );
  });
}

function shouldAcceptDoneResponse({
  coverage,
  feedback,
  decisionsCount,
  ...
  if (
    typeof decisionsCount !== "number" ||
    typeof feedbackStartDecisionsCount !== "number"
  )
    return false;
  const requiredFollowups = estimateRequiredFollowupsFromFeedback(feedback);
  const followupsByCount = Math.max(
    0,
    decisionsCount - feedbackStartDecisionsCount,
  );
  const meaningfulFollowups = collectMeaningfulFollowups(
    decisions,
    feedbackStartDecisionsCount,
  );
  const followupsCollected = Array.isArray(meaningfulFollowups)
    ? meaningfulFollowups.length
    : followupsByCount;
...
function buildQuestionPrompt(
  ...
  decisions,
  feedback,
  { isResume, isBrownfield, coverage, repoContext },
) {
...
function buildRefinementPrompt(
  decisions,
  taskDescription,
  { coverage, repoContext, isBrownfield },
) {
...
function buildAutopilotRefinementPrompt(session) {
  const expected = session.isBrownfield
