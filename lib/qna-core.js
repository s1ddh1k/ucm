...
function hasUnresolvedContradictions(decisions) {
  ...
  for (const d of decisions) {
    ...
    const questionSignal = normalizeCoverageSignal(d && d.question);
    ...
    const answers = answersByQuestion.get(key);
    answers.add(answerSignal);
...
function countMeaningfulFollowups(decisions, feedbackStartDecisionsCount) {
  ...
  for (let i = start; i < decisions.length; i++) {
    const d = decisions[i] || {};
    const questionSignal = normalizeCoverageSignal(d.question);
    const answerSignal = normalizeCoverageSignal(d.requirement || d.answer);

    if (!questionSignal) continue;
    if (!isInformativeCoverageSignal(answerSignal)) continue;

    const key = `${questionSignal}\u0000${answerSignal}`;
...
function shouldAcceptDoneResponse({ coverage, feedback, decisionsCount, feedbackStartDecisionsCount, decisions = [] }) {
  ...
  const meaningfulFollowups = countMeaningfulFollowups(decisions, feedbackStartDecisionsCount);
  const followupsCollected = typeof meaningfulFollowups === "number" ? meaningfulFollowups : followupsByCount;
