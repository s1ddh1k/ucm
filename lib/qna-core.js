...
function normalizeQuestionToken(token) {
  ...
  normalized = normalized.replace(/(?:인가요|인가|입니까|일까요|할까요|나요|까요|요)$/g, "");
  normalized = normalized.replace(/(?:으로|로|은|는|이|가|을|를|과|와|도|만|에|의)$/g, "");
...
const GAP_KEYWORD_STOPWORDS = new Set([
  ...
  "구체", "구체적", "구체적으로", "명확", "명확히", "수준", "명세",
  "검증", "결과", "기준", "조건",
  "the", "and", "for", "with", "from", "that", "this", "spec",
]);
