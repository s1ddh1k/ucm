...
async function saveCurationMode(modeData) {
  ...
  const tmpPath = `${CURATION_MODE_PATH}.tmp`;
  await writeFile(tmpPath, JSON.stringify(modeData, null, 2) + "\n");
  await rename(tmpPath, CURATION_MODE_PATH);
}
...
async function evaluateModeTransition(metrics, config) {
  return withLock("curation-mode", async () => {
    ...
    const autoTransition =
      config?.curation?.autoTransition ?? DEFAULT_CONFIG.curation.autoTransition;
...
async function setMode(mode, reason) {
  if (mode !== "stabilization" && mode !== "big_bet") {
    throw new Error(`invalid mode: ${mode}. Must be "stabilization" or "big_bet"`);
  }
  return withLock("curation-mode", async () => {
    const modeData = await loadCurationMode();
...
const SCORE_AXES = ["impact", "urgency", "uncertainty", "executionCost", "cwFitness"];
...
async function saveClusters(data) {
  ...
  const tmpPath = `${CLUSTERS_PATH}.tmp`;
  await writeFile(tmpPath, JSON.stringify(data, null, 2) + "\n");
  await rename(tmpPath, CLUSTERS_PATH);
}
...
async function clusterProposals(config) {
  ...
  if (proposals.length === 0) return { version: 1, clusters: {}, proposalToCluster: {} };
  ...
  for (const [, catProposals] of Object.entries(byCategory)) {
    ...
    for (const indices of Object.values(groups)) {
      ...
      indices.sort((a, b) => (catProposals[b].priority || 0) - (catProposals[a].priority || 0));
      ...
      clusters[clusterId] = {
        ...
        members: members.map((m, idx) => ({
          ...
          relationship: m.dedupHash === representative.dedupHash ? "duplicate" : "complementary",
        })),
        mergedScores,
...
async function mergeIntoClusters(proposalIds, representativeId) {
  ...
  clusterData.clusters[clusterId] = {
    ...
    members: proposals.map((p) => ({
      ...
      relationship: p.dedupHash === rep.dedupHash ? "duplicate" : "complementary",
    })),
    mergedScores,
    mergedPriority: computePriority(mergedScores, profile.weights),
  };
...
async function splitFromCluster(proposalId) {
  ...
  cluster.members = cluster.members.filter((m) => m.proposalId !== proposalId);
  delete clusterData.proposalToCluster[proposalId];
...
async function loadDeferredProposals() {
  try {
    ...
    data.entries = data.entries.filter((e) => !e.expiresAt || e.expiresAt > now);
    if (data.entries.length < before) {
      log(`[curation] pruned ${before - data.entries.length} expired deferred entries`);
...
async function saveDeferredProposals(data) {
  await mkdir(DAEMON_DIR, { recursive: true });
  const tmpPath = `${DEFERRED_PROPOSALS_PATH}.tmp`;
  await writeFile(tmpPath, JSON.stringify(data, null, 2) + "\n");
  await rename(tmpPath, DEFERRED_PROPOSALS_PATH);
}
...
async function appendCurationLog(entry) {
  ...
  await appendFile(CURATION_LOG_PATH, line);
}

async function recordCurationAction(proposalId, action, previousValue, newValue, actor, reason) {
  const entry = {
    proposalId,
    actor,
    action,
...
async function loadFeedback() {
  try {
    ...
    return { records: [], aggregates: { byCategory: {}, byRisk: {}, byCategoryRisk: {}, lastUpdated: null } };
  }
}
...
async function saveFeedback(data) {
  ...
  await writeFile(tmpPath, JSON.stringify(data, null, 2) + "\n");
  await rename(tmpPath, FEEDBACK_PATH);
}
...
function updateAggregates(feedback) {
  ...
  const activeRecords = feedback.records.filter((r) => {
    const ageDays = (now - new Date(r.executedAt).getTime()) / (1000 * 60 * 60 * 24);
  ...
  for (const record of activeRecords) {
    ...
    const verdict = validVerdicts.has(record.verdict) ? record.verdict : "neutral";
...
function calibrateScores(rawScores, proposal, aggregates) {
  ...
  if (successRate > 0.70 && stats.total >= 5) {
...
async function saveDiscardHistory(data) {
  await mkdir(PROPOSALS_DIR, { recursive: true });
  const tmpPath = `${DISCARD_HISTORY_PATH}.tmp`;
  await writeFile(tmpPath, JSON.stringify(data, null, 2) + "\n");
  await rename(tmpPath, DISCARD_HISTORY_PATH);
}
...
async function discardProposal(proposalId, reason, actor) {
  ...
  const validReasons = ["duplicate", "superseded", "noise", "conflict", "manual", "regulator"];
  ...
  return withLock("discard", async () => {
    ...
    if (history.records.length > maxRecords) {
      history.records = history.records.slice(history.records.length - maxRecords);
    }
...
async function computeReadinessChecklist(proposalId) {
  ...
  const checklist = {
    ...
    successCriteria: {
      passed: !!(proposal.expectedImpact && proposal.expectedImpact.length > 10),
