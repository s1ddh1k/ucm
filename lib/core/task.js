...
function wrapTaskDagError(operation, error, context = {}) {
  const details = Object.entries(context)
    .filter(([, value]) => value !== undefined && value !== null && value !== "")
    .map(([key, value]) => `${key}=${value}`)
    .join(" ");
  const taskSuffix =
    context.taskId !== undefined && context.taskId !== null
      ? `: ${context.taskId}`
      : "";
  const detailSuffix = details ? ` (${details})` : "";
  const wrapped = new Error(
    `${operation} failed${taskSuffix}${detailSuffix}: ${error.message}`,
  );
...
class TaskDag {
  ...
  async _doSave() {
    ...
    try {
      ...
      const wrapped = wrapTaskDagError("task save", e, {
        taskId: this.id,
        filePath,
        stage: "task-save",
      });
  ...
  static async load(id) {
    ...
    try {
      ...
      if (e.code === "ENOENT") {
        const missing = new Error(`task not found: ${id}`);
        missing.code = "ENOENT";
        missing.taskId = id;
        missing.filePath = filePath;
        missing.stage = "task-load";
        missing.retryable = false;
        missing.cause = e;
        throw missing;
      }
      throw wrapTaskDagError("task load", e, {
        taskId: id,
        filePath,
        stage: "task-load",
      });
