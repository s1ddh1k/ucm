...
class TaskDag {
  constructor({ id, status = "pending", pipeline, spec, title, tasks = [], createdAt, startedAt, completedAt, updatedAt, currentStage, stageHistory, warnings, tokenUsage } = {}) {
  ...
  async save() {
    ...
    const current = prev.catch((e) => { console.error(`[TaskDag] save queue error (non-fatal): ${e.message}`); }).then(() => this._doSave());
