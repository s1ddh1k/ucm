...
const { MAX_CONCURRENT_TASKS } = require("./constants");

const CONFLICT_RE = /conflict|CONFLICT|merge failed/i;
const MERGE_REBASE_CAPACITY_CODE = "MERGE_REBASE_CAPACITY";
const MERGE_REBASE_RETRY_DELAY_MS = 1000;
...
class MergeQueueManager {
  ...
  _ensureRebaseCapacity(taskId, project) {
    const activeCount = this.activeForgePipelines?.size || 0;
    if (activeCount >= MAX_CONCURRENT_TASKS) {
      const error = new Error(
        `concurrent task limit reached (${MAX_CONCURRENT_TASKS}). Wait for a task to finish or increase UCM_MAX_CONCURRENT.`,
      );
      error.code = MERGE_REBASE_CAPACITY_CODE;
      error.taskId = taskId;
  ...
  async _handleMergeFailure(entry, queue, project, error) {
    ...
    try {
      ...
      if (!task) throw new Error(`task not found: ${entry.taskId}`);
      const projects = this.normalizeProjects(task);
      this._ensureRebaseCapacity(entry.taskId, project);
      ...
      if (rebaseError?.code === MERGE_REBASE_CAPACITY_CODE) {
        entry.status = "queued";
        queue.current = null;
        queue.entries.unshift(entry);
        this.broadcastWs("mergeQueue:updated", {
        ...
        this.log(
          `[merge-queue] rebase deferred for ${entry.taskId}: ${rebaseError.message}`,
        );
        await this.save();
        setTimeout(
          () => this._scheduleNext(project),
          MERGE_REBASE_RETRY_DELAY_MS,
        );
        return;
      }
