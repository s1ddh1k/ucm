...
class MergeQueueManager {
  constructor({ log, broadcastWs, config, mergeWorktrees, loadWorkspace, removeWorktrees,
    moveTask, updateTaskMeta, loadTask, normalizeProjects, ForgePipeline, wireEvents,
    activeForgePipelines, inflightTasks, markStateDirty, daemonState, drainTaskMetaQueue,
    flushStateNow }) {
  ...
  _cleanupDaemonState(taskId) {
    const ds = typeof this.daemonState === "function" ? this.daemonState() : this.daemonState;
    if (!ds) return;
  ...
  _scheduleNext(project) {
    setImmediate(() => this._processNext(project).catch((e) => {
      this.log(`[merge-queue] processNext error for ${project}: ${e.message}`);
  ...
  enqueue(taskId, project, priority = 0) {
    ...
    if (queue.entries.length >= (mqConfig.maxQueueSize || 20)) {
      throw new Error(`merge queue full for project ${project} (max: ${mqConfig.maxQueueSize || 20})`);
    }
    ...
    if (this.updateTaskMeta) {
      this.updateTaskMeta(taskId, { mergeQueue: "queued" }).catch((e) =>
        this.log(`[merge-queue] updateTaskMeta error for ${taskId}: ${e.message}`));
    }
    ...
    this.broadcastWs("mergeQueue:updated", {
      project, queueLength: queue.entries.length, processing: !!queue.current,
    });

    this.log(`[merge-queue] enqueued ${taskId} for ${project} (queue: ${queue.entries.length})`);
    this.save().catch((e) => this.log(`[merge-queue] save error: ${e.message}`));
  ...
  async _processNext(project) {
    ...
    if (this.updateTaskMeta) {
      this.updateTaskMeta(entry.taskId, { mergeQueue: "merging" }).catch((e) =>
        this.log(`[merge-queue] updateTaskMeta error for ${entry.taskId}: ${e.message}`));
    }
    ...
    try {
      ...
      this.broadcastWs("mergeQueue:merged", { taskId: entry.taskId, project, durationMs });
      this.broadcastWs("mergeQueue:updated", {
        project, queueLength: queue.entries.length, processing: false,
      });
      this.log(`[merge-queue] merged ${entry.taskId} for ${project} (${durationMs}ms)`);
      ...
      try {
        ...
        this.log(`[merge-queue] fatal error handling merge failure for ${entry.taskId}: ${fatalError.message}`);
  ...
  async _handleMergeFailure(entry, queue, project, error) {
    this.log(`[merge-queue] merge failed for ${entry.taskId}: ${error.message}`);
    const mqConfig = this._getConfig();
    ...
    if (!isConflict || conflictResolution === "manual" || entry.rebaseCount >= maxRebaseAttempts) {
      await this._failEntry(entry, queue, project, error.message);
      return;
    }

    this.broadcastWs("mergeQueue:conflict", {
      taskId: entry.taskId, project,
    ...
    this.broadcastWs("mergeQueue:rebasing", {
      taskId: entry.taskId, project,
      attempt: entry.rebaseCount, maxAttempts: maxRebaseAttempts,
    });
    this.log(`[merge-queue] rebasing ${entry.taskId} (attempt ${entry.rebaseCount}/${maxRebaseAttempts})`);
    ...
    try {
      ...
      if (status === "done" || status === "auto_merged" || status === "merge_queued") {
        // re-queue for merge attempt
        entry.status = "queued";
        this.log(`[merge-queue] rebase complete for ${entry.taskId}, re-attempting merge`);
        ...
        this.broadcastWs("mergeQueue:failed", {
          taskId: entry.taskId, project, error: `rebase pipeline status: ${status}`,
        });

        try {
          await this.moveTask(entry.taskId, "running", status === "review" ? "review" : "failed", {
            mergeQueue: status === "review" ? "conflict" : null,
            mergeRebaseCount: null,
          });
        } catch (moveError) {
          this.log(`[merge-queue] moveTask error for ${entry.taskId}: ${moveError.message}`);
        }
      ...
      this.log(`[merge-queue] rebase error for ${entry.taskId}: ${rebaseError.message}`);
      this.activeForgePipelines?.delete(entry.taskId);
      this.inflightTasks?.delete(entry.taskId);
  ...
  async _failEntry(entry, queue, project, errorMessage) {
    ...
    this.broadcastWs("mergeQueue:failed", {
      taskId: entry.taskId, project, error: errorMessage,
    });
    this.broadcastWs("mergeQueue:updated", {
      project, queueLength: queue.entries.length, processing: false,
    });

    this.log(`[merge-queue] ${entry.taskId} failed (${entry.rebaseCount} rebase attempt(s)), moving to review`);
    try {
      ...
      this.log(`[merge-queue] moveTask to review failed for ${entry.taskId}: ${moveError.message}`);
    }
  ...
  remove(taskId) {
    for (const [project, queue] of this.projectQueues) {
      ...
      if (queue.current && queue.current.taskId === taskId) {
        this.log(`[merge-queue] cannot remove ${taskId}: currently ${queue.current.status}`);
        return null;
      }
  ...
  skip(taskId) {
    ...
    if (removed) {
      ...
      if (this.updateTaskMeta) {
        this.updateTaskMeta(taskId, { mergeQueue: null, mergeRebaseCount: null }).catch(() => {});
      }
    }
    return !!removed;
  }
  ...
  getStatus(project) {
    if (project) {
      const queue = this.projectQueues.get(project);
      if (!queue) return { project, processing: false, current: null, entries: [], queueLength: 0 };
      return {
        project,
        processing: !!queue.current,
        current: queue.current ? {
          taskId: queue.current.taskId, status: queue.current.status,
          priority: queue.current.priority, enqueuedAt: queue.current.enqueuedAt,
          rebaseCount: queue.current.rebaseCount,
        } : null,
        entries: queue.entries.map((e) => ({
          taskId: e.taskId, status: e.status, priority: e.priority,
          enqueuedAt: e.enqueuedAt, rebaseCount: e.rebaseCount,
        })),
  ...
  async load() {
    ...
    try {
      ...
      if (data && typeof data === "object") {
        ...
        this.log(`[merge-queue] loaded ${this.projectQueues.size} project queue(s)`);
  ...
  async save() {
    ...
    this._saveQueue = op.catch((e) => this.log(`[merge-queue] save queue error: ${e.message}`));
    return op;
  }
}
