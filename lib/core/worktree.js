...
async function acquireLock(taskId) {
  ...
  try {
    ...
    const timedOut = Date.now() - stat.mtimeMs > LOCK_TIMEOUT_MS;
    let isStale = false;
...
function releaseLock(lockRef) {
  ...
  try {
    if (ownerToken) {
      const current = parseLockOwner(fs.readFileSync(lockPath, "utf-8"));
    ...
    if (e.code !== "ENOENT")
      console.error(
        `[releaseLock] failed to release lock for ${taskId}: ${e.message}`,
      );
  }
}
...
async function cleanupTask(taskId) {
  ...
  try {
    ...
    console.error(
      `[cleanupTask] worktree cleanup failed for ${taskId}, falling back to rm: ${e.message}`,
    );
    try {
