function quoteUserContent(text) {
  if (typeof text !== "string") return String(text ?? "");
  // Escape markdown heading markers that could be mistaken for prompt structure
...
const MAX_INLINE_DECISIONS = 7;

function summarizeDecisions(decisions) {
  if (decisions.length <= MAX_INLINE_DECISIONS) {
    return decisions
      .map(
        (d) =>
          `- **[${d.area}]** ${quoteUserContent(d.question)} → ${quoteUserContent(d.answer)}`,
      )
      .join("\n");
  }
  const old = decisions.slice(0, -MAX_INLINE_DECISIONS);
  const recent = decisions.slice(-MAX_INLINE_DECISIONS);
  const byArea = {};
  for (const d of old) {
    byArea[d.area] = (byArea[d.area] || 0) + 1;
  }
  let summary = `(이전 결정 ${old.length}개 요약)\n`;
  summary += Object.entries(byArea)
    .map(([a, c]) => `- ${a}: ${c}개 결정 완료`)
    .join("\n");
  summary += `\n\n최근 결정:\n`;
  summary += recent
...
## 고정 영역 (이 이름만 사용 — 다른 이름 사용 시 파싱 실패)
...
## 응답 형식 (반드시 JSON만 출력 — 마크다운, 설명, 코드 금지)

### 질문 예시:
{
  "question": "인증 방식으로 무엇을 사용할까요?",
  "options": [
    { "label": "JWT 토큰 기반 인증", "reason": "stateless하여 수평 확장에 유리" },
    { "label": "세션 쿠키 기반 인증", "reason": "서버 측 세션으로 즉시 무효화 가능" },
    { "label": "OAuth 2.0 위임 인증", "reason": "소셜 로그인 통합에 적합" }
  ],
  "area": "설계 결정",
  "done": false
}
...
- 작업 목표 영역에 3개 이상 질문 금지. 목표 확인은 1-2개 질문으로 끝내세요.`;
  ...
  if (decisions.length > 0) {
    ...
    prompt += summarizeDecisions(decisions);
  }
...
### 질문 예시:
{
  "question": "이 API의 에러 응답 형식은 어떻게 해야 할까요?",
  "options": [
    { "label": "HTTP 상태 코드 + JSON 에러 본문", "reason": "REST 표준에 부합" },
    { "label": "항상 200 + 응답 내 error 필드", "reason": "클라이언트 파싱 단순화" },
    { "label": "GraphQL 스타일 errors 배열", "reason": "부분 성공/실패 표현 가능" }
  ],
  "area": "기능 요구사항",
  "done": false
}
...
${Object.entries(coverage)
  ...
  if (session.decisions.length > 0) {
    prompt += `\n\n## 지금까지 수집된 결정\n\n`;
    prompt += summarizeDecisions(session.decisions);
  }
...
### 예시:
{
  "question": "캐시 무효화 전략은 무엇으로 할까요?",
  "area": "기능 요구사항",
  "answer": "TTL 기반 자동 만료 + 수동 무효화 API",
  "reason": "대부분의 경우 TTL로 충분하고, 긴급 시 수동 무효화 가능",
  "requirement": "캐시는 TTL 기반 자동 만료와 수동 무효화 API를 모두 지원해야 한다",
  "done": false
}
...
### 거부되는 응답: area가 목록에 없거나, JSON이 아닌 텍스트`;
