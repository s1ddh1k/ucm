...
function killPidTree(pid) {
  ...
  try {
    ...
    if (e.code !== "ESRCH")
      console.error(
        `[killPidTree] SIGTERM pgid ${pid}: ${e.code || e.message}`,
      );
  }
  try {
    process.kill(pid, "SIGTERM");
    ...
    if (e.code !== "ESRCH")
      console.error(`[killPidTree] SIGTERM pid ${pid}: ${e.code || e.message}`);
  }
  const fallback = setTimeout(() => {
    ...
    try {
      ...
      if (e.code !== "ESRCH")
        console.error(
          `[killPidTree] SIGKILL pgid ${pid}: ${e.code || e.message}`,
        );
    }
    try {
      process.kill(pid, "SIGKILL");
      ...
      if (e.code !== "ESRCH")
        console.error(
          `[killPidTree] SIGKILL pid ${pid}: ${e.code || e.message}`,
        );
    }
  }, 1200);
  fallback.unref();
  ...
  return new Promise((resolve) => {
    ...
    child.stdin.on("error", (e) => {
      if (e.code !== "EPIPE" && e.code !== "ERR_STREAM_DESTROYED")
        console.error(`[llm] stdin error: ${e.code || e.message}`);
    });
    child.stdout.on("error", (e) => {
      if (e.code !== "ERR_STREAM_DESTROYED")
        console.error(`[llm] stdout error: ${e.code || e.message}`);
    });
    child.stderr.on("error", (e) => {
      if (e.code !== "ERR_STREAM_DESTROYED")
        console.error(`[llm] stderr error: ${e.code || e.message}`);
    });
