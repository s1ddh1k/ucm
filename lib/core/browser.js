...
function killBrowser(browser) {
  ...
  } catch (e) { if (e.code !== "ESRCH") console.error(`[browser] SIGTERM pid ${browser.process.pid}: ${e.code || e.message}`); }
  setTimeout(() => {
    try { process.kill(browser.process.pid, "SIGKILL"); } catch (e) { if (e.code !== "ESRCH") console.error(`[browser] SIGKILL pid ${browser.process.pid}: ${e.code || e.message}`); }
  }, 1500);
  try { fs.rmSync(browser.profileDir, { recursive: true, force: true }); } catch (e) { if (e.code !== "ENOENT") console.error(`[browser] rmSync ${browser.profileDir}: ${e.code || e.message}`); }
}
  ...
  return {
    ...
    kill() {
      try { process.kill(-child.pid, "SIGTERM"); } catch (e) {
        if (e.code !== "ESRCH") console.error(`[devServer] SIGTERM pgid ${child.pid}: ${e.code || e.message}`);
        try { child.kill("SIGTERM"); } catch (e2) { if (e2.code !== "ESRCH") console.error(`[devServer] SIGTERM pid ${child.pid}: ${e2.code || e2.message}`); }
      }
    },
  };
}
...
module.exports = {
  ...
  // 테스트용 export
  extractPortFromScript, pickDevScript, scriptLooksLikeWebServer, splitCommandString,
