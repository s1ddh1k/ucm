...
function validateWebDist(distDir, opts = {}) {
  ...
  try {
    ...
    if (!indexStat.isFile()) {
      return {
        ok: false,
        reason: "index.html is not a file",
        missingAssets: [],
      };
    }
    ...
    return {
      ok: false,
      reason: `index.html missing (${e.message})`,
      missingAssets: [],
    };
  }
  ...
  if (!html.includes('id="root"')) {
    return {
      ok: false,
      reason: "index.html missing root mount node",
      missingAssets: [],
    };
  }

  const refs = collectAssetRefs(html);
  const scripts = refs.filter(
    (ref) => ref.endsWith(".js") || ref.endsWith(".mjs"),
  );
  if (requireScript && scripts.length === 0) {
    return {
      ok: false,
      reason: "no bundled script reference in index.html",
      missingAssets: [],
    };
  }
