...
function spawnAgent(
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (e.code !== "EEXIST")
        console.error(
          `[agent] failed to create task log ${taskLogPath}: ${e.code || e.message}`,
        );
      taskLogStream = null;
    }
    try {
      ...
      if (e.code !== "EEXIST")
        console.error(
          `[agent] failed to create stage log ${stageLogPath}: ${e.code || e.message}`,
        );
      stageLogStream = null;
    }
      ...
      if (outputFormat === "stream-json") {
        ...
        while ((newlineIdx = stdoutBuf.indexOf("\n")) !== -1) {
          ...
          try {
            ...
            if (event.type === "assistant" && event.message?.content) {
              for (const block of event.message.content) {
                if (block.type === "text" && block.text) {
                  ...
                  const line = sanitizeContent(
                    `[tool] ${block.name}${detail ? ` — ${detail}` : ""}`,
                  );
                  writeLog(line);
                  if (onLog) onLog(line);
            ...
            /* malformed stream-json line, skip */
          }
        }
        ...
        while ((newlineIdx = stdoutBuf.indexOf("\n")) !== -1) {
          ...
          try {
            ...
            if (item.type === "reasoning" && typeof item.text === "string") {
              const line = sanitizeContent(
                `[thinking] ${item.text.trim().slice(0, 300)}`,
              );
              if (line.trim()) {
                writeLog(line);
                if (onLog) onLog(line);
            ...
            if (item.type === "command_execution") {
              ...
              if (
                ...
                const outLines = item.aggregated_output
                  .replace(/\r/g, "\n")
                  .split("\n");
    ...
    child.stdin.on("error", (e) => {
      if (e.code !== "EPIPE" && e.code !== "ERR_STREAM_DESTROYED")
        writeLog(`[agent] stdin error: ${e.code || e.message}`);
    });
    child.stdout.on("error", (e) => {
      if (e.code !== "ERR_STREAM_DESTROYED")
        writeLog(`[agent] stdout error: ${e.code || e.message}`);
    });
    child.stderr.on("error", (e) => {
      if (e.code !== "ERR_STREAM_DESTROYED")
        writeLog(`[agent] stderr error: ${e.code || e.message}`);
    });
