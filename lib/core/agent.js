  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      taskLogStream.on("error", () => {
        try { taskLogStream.end(); } catch { /* stream cleanup */ }
        taskLogStream = null;
      });
    } catch (e) {
      if (e.code !== "EEXIST") console.error(`[agent] failed to create task log ${taskLogPath}: ${e.code || e.message}`);
      taskLogStream = null;
    }
    try {
      ...
      stageLogStream.on("error", () => {
        try { stageLogStream.end(); } catch { /* stream cleanup */ }
        stageLogStream = null;
      });
    } catch (e) {
      if (e.code !== "EEXIST") console.error(`[agent] failed to create stage log ${stageLogPath}: ${e.code || e.message}`);
      stageLogStream = null;
    }
    ...
    function closeStreams() {
      if (taskLogStream) { try { taskLogStream.end(); } catch { /* stream cleanup */ } taskLogStream = null; }
      if (stageLogStream) { try { stageLogStream.end(); } catch { /* stream cleanup */ } stageLogStream = null; }
    }
    ...
    child.stdin.on("error", (e) => { if (e.code !== "EPIPE" && e.code !== "ERR_STREAM_DESTROYED") writeLog(`[agent] stdin error: ${e.code || e.message}`); });
    child.stdout.on("error", (e) => { if (e.code !== "ERR_STREAM_DESTROYED") writeLog(`[agent] stdout error: ${e.code || e.message}`); });
    child.stderr.on("error", (e) => { if (e.code !== "ERR_STREAM_DESTROYED") writeLog(`[agent] stderr error: ${e.code || e.message}`); });
