...
function startForgePipeline(fp, taskId, logPrefix = "forge") {
  ensureForgeCapacity();
  ...
  const socketHandlers = {
    ...
    resume: async (p) => {
      if (p && p.taskId) {
        ...
        if (!p.project) throw new Error("resume requires project");
        const { ForgePipeline, assertResumableDagStatus } = require("./forge/index");
