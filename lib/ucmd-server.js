...
const { SOCK_PATH, MAX_SOCKET_REQUEST_BYTES } = require("./ucmd-constants.js");
...
function ensureForgeCapacity() {
  ...
  if (activeCount >= MAX_CONCURRENT_TASKS) {
    throw new Error(
      `concurrent task limit reached (${MAX_CONCURRENT_TASKS}). Wait for a task to finish or increase UCM_MAX_CONCURRENT.`,
    );
  }
}
...
function startForgePipeline(fp, taskId, logPrefix = "forge", hooks = {}) {
  ...
  fp.run()
    .then(async (result) => {
      ...
      if (typeof hooks.onSuccess === "function") {
        try {
          ...
          deps.log?.(
            `[${logPrefix}] onSuccess hook error for ${taskId}: ${e.message}`,
          );
        }
      }
    })
    .catch(async (e) => {
      if (deps.log)
        deps.log(`[${logPrefix}] error for ${taskId}: ${e.message}`);
      await deps.drainTaskMetaQueue?.(taskId);
      ...
      if (typeof hooks.onError === "function") {
        try {
          ...
          deps.log?.(
            `[${logPrefix}] onError hook error for ${taskId}: ${hookError.message}`,
          );
        }
      }
    })
    .finally(async () => {
      ...
      if (typeof hooks.onFinally === "function") {
        try {
          ...
          deps.log?.(
            `[${logPrefix}] onFinally hook error for ${taskId}: ${e.message}`,
          );
        }
      }
    });
...
function probeSocketState() {
  return new Promise((resolve) => {
    ...
    setTimeout(() => finish("unknown"), 500);
  });
}

async function handleSocketServerError(error) {
  if (error?.code !== "EADDRINUSE") {
    throw error;
  }
  const socketState = await probeSocketState();
  if (socketState === "active") {
    throw new Error(
      `socket already in use: ${SOCK_PATH}. another daemon may be running`,
    );
  }
  if (socketState !== "stale") {
    throw new Error(
      `failed to determine socket ownership for ${SOCK_PATH}; refusing to unlink`,
    );
  }

  try {
    fs.unlinkSync(SOCK_PATH);
    deps.log?.(`[socket] removed stale socket: ${SOCK_PATH}`);
  } catch (e) {
    if (e?.code !== "ENOENT") {
      throw new Error(`failed to remove stale socket ${SOCK_PATH}: ${e.message}`);
    }
  }
}
...
function classifySocketError(error) {
  ...
  if (code === "ENOENT" || code === "EAGAIN" || code === "EINTR") {
    return "retryable";
  }
  return "fatal";
}

function socketRequestContext(params) {
  if (!params || typeof params !== "object" || Array.isArray(params)) {
    return "";
  }
  const contextParts = [];
  if (params.taskId) contextParts.push(`taskId=${params.taskId}`);
  if (params.stage) contextParts.push(`stage=${params.stage}`);
  if (params.filePath) contextParts.push(`filePath=${params.filePath}`);
  if (params.path) contextParts.push(`path=${params.path}`);
  if (params.project) contextParts.push(`project=${params.project}`);
...
function validateTaskIdParam(params) {
  ...
  if (!Object.prototype.hasOwnProperty.call(params, "taskId")) {
    return;
  }
  if (typeof params.taskId !== "string" || !TASK_ID_RE.test(params.taskId)) {
    throw new Error(`invalid taskId format: ${params.taskId}`);
  }
}
...
async function handleSocketRequest(line, conn) {
  ...
  const socketHandlers = {
    ...
    resume: async (p) => {
      if (p?.taskId) {
        ...
        if (
          task &&
          (task.state === "review" || task.state === "failed") &&
          typeof deps.moveTask === "function"
        ) {
        ...
        if (
          ...
          task.state === "running" &&
          task.suspended === true &&
          typeof deps.updateTaskMeta === "function"
        ) {
        ...
        const fp = new ForgePipeline({
          ...
          resumeFrom:
            p.fromStage || lastFailedStage(existingDag) || "implement",
        });
        let resumeMergeQueued = false;
        try {
          startForgePipeline(fp, p.taskId, "resume", {
            onSuccess: async (dag) => {
              ...
              if (status === "merge_queued") {
                // merge queue will handle transitions — don't move task
                resumeMergeQueued = true;
                return;
              }
              ...
              try {
                ...
                deps.log?.(
                  `[resume] final transition failed for ${p.taskId}: ${e.message}`,
                );
              }
            },
            onError: async () => {
              ...
              try {
                ...
                deps.log?.(
                  `[resume] onError transition failed for ${p.taskId}: ${e.message}`,
                );
              }
            },
            onFinally: async () => {
              // re-add to activeTasks if merge-queued (clearDaemonTaskTracking already ran)
              if (resumeMergeQueued) {
                markDaemonTaskRunning(p.taskId);
              }
            },
          });
          ...
          if (
            ...
            if (clearedSuspendedMeta.suspendedStage != null) {
              restoreSuspendedMeta.suspendedStage =
                clearedSuspendedMeta.suspendedStage;
            }
            if (clearedSuspendedMeta.suspendedReason != null) {
              restoreSuspendedMeta.suspendedReason =
                clearedSuspendedMeta.suspendedReason;
            }
            try {
              ...
              deps.log?.(
                `[resume] suspended-meta rollback failed for ${p.taskId}: ${rollbackError.message}`,
              );
            }
          }
          if (
            ...
            try {
              ...
              deps.log?.(
                `[resume] rollback failed for ${p.taskId}: ${rollbackError.message}`,
              );
            }
          }
          throw e;
    ...
    proposal_score: (p) => h.handleProposalScore(p),
    proposal_score_set: (p) => h.handleProposalScoreSet(p),
    proposal_clusters: (p) => h.handleProposalClusters(p),
    proposal_cluster_merge: (p) => h.handleProposalClusterMerge(p),
    proposal_cluster_split: (p) => h.handleProposalClusterSplit(p),
    proposal_conflicts: (p) => h.handleProposalConflicts(p),
    proposal_discard: (p) => h.handleProposalDiscard(p),
    discard_history: (p) => h.handleDiscardHistory(p),
    bigbet_checklist: (p) => h.handleBigBetChecklist(p),
    proposal_feedback: (p) => h.handleProposalFeedback(p),
    scoring_profile: (p) => h.handleScoringProfile(p),
    curation_mode: () => h.handleCurationMode(),
    curation_set_mode: (p) => h.handleSetCurationMode(p),
    curation_weights: (p) => h.handleCurationWeights(p),
    ...
    refinement_answer: (p) => {
      ...
      if (
        nestedAnswer &&
        typeof nestedAnswer === "object" &&
        !Array.isArray(nestedAnswer)
      ) {
        const { answer: nestedAnswerAlias, ...normalizedNestedAnswer } =
          nestedAnswer;
        if (
      ...
      if (
        isMissingAnswerValue(flatAnswer.value) &&
        typeof answerAlias === "string"
      ) {
    ...
    set_automation: async (p) => {
      const config = deps.config?.() || {};
      const current = config.automation || {};
      const merged = { ...current, ...p };
      return h.handleSetConfig({ automation: merged });
    },
    update_priority: (p) => h.handleUpdatePriority(p),
    merge_queue_status: (p) => h.handleMergeQueueStatus(p),
    merge_queue_retry: (p) => h.handleMergeQueueRetry(p),
    merge_queue_skip: (p) => h.handleMergeQueueSkip(p),
    forge: async (p) => {
  ...
  if (method === "subscribe") {
    ...
    conn.write(
      `${JSON.stringify({
        event: "daemon:status",
        data: { status: daemonState?.daemonStatus || "running" },
      })}\n`,
    );
  ...
  try {
    validateTaskIdParam(params);
    const result = await handler(params || {});
    conn.end(`${JSON.stringify({ id, ok: true, data: result })}\n`);
  } catch (e) {
    const severity = classifySocketError(e);
    const context = socketRequestContext(params);
    deps.log?.(
      `[socket] ${method}${context} ${severity} error: ${e.message}`,
    );
