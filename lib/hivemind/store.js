...
const RECALL_LOG_PATH = path.join(DAEMON_DIR, "recall-log.jsonl");
  ...
  if (config.models && typeof config.models === "object") {
    for (const [slot, model] of Object.entries(config.models)) {
      if (!isModelCompatible(provider, model)) {
        warnings.push(
          `models.${slot}: invalid model "${model}" for provider "${provider}"`,
        );
      }
    }
  }
  ...
  for (const [field, [min, max]] of Object.entries(numericFields)) {
    if (config[field] !== undefined) {
      if (
        ...
        warnings.push(
          `${field}: expected number ${min}-${max}, got "${config[field]}"`,
        );
      }
    }
  }
...
function loadConfig() {
  ...
  const provider = resolveProvider(
    parsed.llmProvider || DEFAULT_CONFIG.llmProvider,
  );
...
function parseYaml(text) {
  ...
  for (const line of lines) {
    ...
    const listItemInMap = line.match(/^ {4}(\w[\w-]*): (.+)$/);
    ...
    if (typeof value === "object" && !Array.isArray(value)) {
      ...
      for (const [k, v] of Object.entries(value)) {
        if (typeof v === "object" && !Array.isArray(v) && v !== null) {
          ...
          lines.push(
            `  ${displayKey}: ${typeof v === "string" && v.includes(":") ? JSON.stringify(v) : v}`,
          );
        }
      }
      ...
      for (const item of value) {
        if (typeof item === "object" && item !== null) {
          ...
          for (let i = 1; i < entries.length; i++) {
            lines.push(
              `    ${entries[i][0]}: ${JSON.stringify(entries[i][1])}`,
            );
          }
        } else {
      ...
      const needsQuote =
        typeof value === "string" &&
        (value.includes(":") || value.includes("#") || /^\d+$/.test(value));
