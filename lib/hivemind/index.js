...
function cmdConfig(subcommand, value) {
  ...
  if (subcommand === "provider") {
    ...
    if (!normalized) {
      console.error(`Invalid provider: ${value} (expected: ${PROVIDERS.join("|")})`);
      process.exit(1);
    }
    config.llmProvider = normalized;
    const normalizedModels = normalizeModelsForProvider(config.models, normalized);
    config.models = normalizedModels.models;
    store.saveConfig(config);
    console.log(`Updated llmProvider: ${normalized}`);
    if (normalizedModels.replaced.length > 0) {
      console.log(`Reset incompatible model slots: ${normalizedModels.replaced.join(", ")}`);
    }
    return;
  }
...
async function cmdInit() {
  ...
  const providerAnswer = await prompt(`LLM provider (${providerChoices}, default: ${preferredProvider}): `);
  const selectedProvider = normalizeProvider(providerAnswer || preferredProvider) || preferredProvider;
    ...
    switch (command) {
      ...
      case "ingest":
        if (flags.adapter || positional[0]) {
          ...
          const fallback = ["codex", "claude", "document"].find((name) => enabled[name]?.enabled !== false) || "claude";
          await cmdIngest(enabled[preferred]?.enabled === false ? fallback : preferred);
        }
        break;
