...
function log(msg) {
  ...
  try {
    ...
    process.stderr.write(`[hivemind] log write failed: ${e.message} | ${line}\n`);
  }
}
...
function startSocketServer() {
  if (fs.existsSync(store.SOCK_PATH)) {
    try { fs.unlinkSync(store.SOCK_PATH); } catch (e) {
      if (e.code !== "ENOENT") log(`Failed to remove stale socket: ${e.message}`);
    }
  }
    ...
    conn.on("data", (chunk) => {
      ...
      handleRequest(request).then((result) => {
        ...
        try { conn.write(JSON.stringify({ ok: false, error: e.message }) + "\n"); } catch (writeErr) {
          log(`Socket write error (error response for: ${e.message}): ${writeErr.message}`);
        }
        conn.end();
      });
    });
...
function gracefulShutdown() {
  ...
  if (socketServer) {
    socketServer.close();
    try { fs.unlinkSync(store.SOCK_PATH); } catch (e) {
      if (e.code !== "ENOENT") log(`Failed to remove socket on shutdown: ${e.message}`);
    }
  }

  try { fs.unlinkSync(store.PID_PATH); } catch (e) {
    if (e.code !== "ENOENT") log(`Failed to remove PID file on shutdown: ${e.message}`);
  }
  indexer.close();
...
function stopDaemon() {
  ...
  try {
    ...
    if (e.code === "ESRCH") {
      ...
      try { fs.unlinkSync(store.PID_PATH); } catch (e) {
        if (e.code !== "ENOENT") console.error(`Failed to remove stale PID file: ${e.message}`);
      }
    } else {
      throw e;
    }
