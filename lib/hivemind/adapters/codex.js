...
function parseResponseItem(entry, messages) {
  ...
  if (payload.type === "message") {
    ...
    if (role === "assistant") {
      pushMessage(
        messages,
        "assistant",
        textFromBlocks(content, ["output_text", "text"]),
      );
      return;
    }
    if (role === "user") {
      pushMessage(
        messages,
        "user",
        textFromBlocks(content, ["input_text", "text"]),
      );
    }
    return;
  }
  ...
  if (payload.type === "reasoning") {
    pushMessage(
      messages,
      "reasoning",
      flattenText(payload.summary) || flattenText(payload.text),
    );
  }
}
      ...
      try {
        ...
        if (entry.type === "user_message")
          pushMessage(
            messages,
            "user",
            entry.payload?.content || entry.payload?.text,
          );
