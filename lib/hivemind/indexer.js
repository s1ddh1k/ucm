...
// Korean particle stripping for keyword normalization
const KO_PARTICLES_RE = /(?:은|는|이|가|을|를|의|에|로|으로|와|과|도|만|부터|까지|에서|처럼|보다|라|이라)$/;

function stripParticle(keyword) {
  if (keyword.length <= 2) return keyword;
  const stripped = keyword.replace(KO_PARTICLES_RE, "");
  return stripped.length >= 2 ? stripped : keyword;
}
...
function getDb() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS search_expansions (
      term      TEXT NOT NULL,
      zettel_id TEXT NOT NULL,
      PRIMARY KEY (term, zettel_id)
    );

    CREATE INDEX IF NOT EXISTS idx_expansions_term ON search_expansions(term);
  `);
...
function prepareStatements() {
  stmts = {
    ...
    insertExpansion: db.prepare(
      "INSERT OR REPLACE INTO search_expansions (term, zettel_id) VALUES (?, ?)",
    ),
    deleteExpansions: db.prepare(
      "DELETE FROM search_expansions WHERE zettel_id = ?",
    ),
    selectExpansion: db.prepare(
      "SELECT zettel_id AS id FROM search_expansions WHERE term = ?",
    ),
    selectAllExpansionTerms: db.prepare(
      "SELECT DISTINCT term FROM search_expansions",
    ),
    selectAllKeywordTerms: db.prepare(
...
function insertZettelRow(zettel) {
  ...
  stmts.deleteExpansions.run(zettel.id);
  ...
  // Insert keywords + tokens (with Korean particle stripping)
  for (const [keyword, weight] of Object.entries(keywords)) {
    const normalized = stripParticle(keyword.toLowerCase());
  ...
  const keywordsText = Object.keys(keywords)
    .map((k) => stripParticle(k.toLowerCase()))
    .join(" ");
  stmts.insertFts.run(
    zettel.id,
    zettel.title || "",
...
function buildFromDisk() {
  ...
  d.exec("DELETE FROM search_expansions");
  ...
  const insertAll = d.transaction(() => {
    for (const file of files) {
      ...
      if (!zettel.title) zettel.title = path.basename(file, ".md");
      if (!zettel.createdAt) zettel.createdAt = new Date().toISOString();
      if (!zettel.lastAccessed) zettel.lastAccessed = zettel.createdAt;
      insertZettelRow(zettel);
      if (zettel.searchTerms?.length) {
        for (const term of zettel.searchTerms) {
          const normalized = term.toLowerCase().trim();
          if (normalized) stmts.insertExpansion.run(normalized, zettel.id);
        }
      }
    }
  });
...
function lookupKeywordsWeighted(keywords, queryWeights) {
  ...
  for (const keyword of keywords) {
    const lower = stripParticle(keyword.toLowerCase());
    if (seen.has(lower)) continue;
    seen.add(lower);
    const queryWeight = queryWeights?.[keyword.toLowerCase()] || queryWeights?.[lower] || 1;
    ...
    // 4. Search expansion match
    const expansionRows = stmts.selectExpansion.all(lower);
    for (const { id } of expansionRows) {
      scores[id] = (scores[id] || 0) + queryWeight * 0.5;
    }
  }
...
function indexExpansions(zettelId, terms) {
  getDb();
  stmts.deleteExpansions.run(zettelId);
  for (const term of terms) {
    const normalized = term.toLowerCase().trim();
    if (normalized) stmts.insertExpansion.run(normalized, zettelId);
  }
}

function getAllExpansionTerms() {
  getDb();
  const rows = stmts.selectAllExpansionTerms.all();
  return new Set(rows.map((r) => r.term));
}

function getAllKeywordTerms() {
  getDb();
  const rows = stmts.selectAllKeywordTerms.all();
