...
async function validate(opts) {
  ...
  if (opts.decisions) {
    try {
      await access(path.resolve(opts.decisions));
    } catch {
      errors.push(`decisions 파일 없음: ${opts.decisions}`);
    }
  }
  if (opts.template) {
    try {
...
async function main() {
  ...
  const outputDir = opts.output
    ...
    : path.join("/tmp", `spec-${runId}`);
  await mkdir(outputDir, { recursive: true });

  const decisionsContent = await readFile(
    path.resolve(opts.decisions),
    "utf-8",
  );
  ...
  let requirements;
  try {
    requirements = await generateRequirements(decisions, {
      template,
      cwd,
      provider,
    });
