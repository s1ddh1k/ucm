...
const { buildCommand } = require("./core/llm");
...
const { setMergeQueueManager: setDeliverMergeQueue } = require("./forge/deliver.js");
...
const {
  ...
  SOCK_PATH, PID_PATH, LOG_PATH, STATE_PATH, MERGE_QUEUE_PATH,
...
const {
  ...
  handleProposalEvaluate, handleProposalDelete, handleSnapshots,
...
const {
  ...
  handleMergeQueueStatus, handleMergeQueueRetry, handleMergeQueueSkip,
} = ucmdHandlers;
...
async function loadConfig() {
  try {
    ...
    if (e && e.code === "ENOENT") {
      ...
      log(`loadConfig: failed to load ${CONFIG_PATH}: ${e.message} — using defaults in memory`);
      config = { ...DEFAULT_CONFIG };
    }
  }
...
async function updateTaskProject(taskId, projectPath) {
  for (const state of TASK_STATES) {
    ...
    try {
      ...
      if (e.code !== "ENOENT") console.error(`[updateTaskProject] failed for ${taskId} in state ${state}: ${e.message}`);
    }
  }
}
...
async function updateTaskMeta(taskId, updates) {
  const prev = _metaUpdateQueue.get(taskId) || Promise.resolve();
  const current = prev.catch(() => {}).then(async () => {
    for (const state of TASK_STATES) {
      ...
      try {
        ...
        if (e.code !== "ENOENT") console.error(`[updateTaskMeta] failed for ${taskId} in state ${state}: ${e.message}`);
      }
    }
  });
  _metaUpdateQueue.set(taskId, current);
  current.finally(() => {
    if (_metaUpdateQueue.get(taskId) === current) _metaUpdateQueue.delete(taskId);
  });
  return current;
}
...
async function truncateLogIfNeeded() {
  ...
  } catch (e) { log(`[warning] log truncation failed: ${e.message}`); }
}
...
function mergeStateStats(state) {
  ...
  const savedStats = state && typeof state.stats === "object" && state.stats !== null
    ? state.stats
    : {};
...
async function loadState() {
  try {
    ...
    if (!Array.isArray(daemonState.suspendedTasks)) daemonState.suspendedTasks = [];
    if (typeof daemonState.daemonStatus !== "string") daemonState.daemonStatus = "running";
  } catch (e) {
...
async function flushState() {
  ...
  _flushQueue = op.catch((e) => { log(`flushState queue error (non-fatal): ${e.message}`); });
...
async function flushStateNow() {
  ...
  stateDirty = true;
  stateGeneration++;
  try { await flushState(); } catch (e) { log(`critical state flush error: ${e.message}`); }
}
...
async function findOrphanWorktrees() {
  ...
  } catch (e) { if (e && e.code !== "ENOENT") log(`findOrphanWorktrees: readdir failed: ${e.message}`); }
  return orphans;
}
...
async function performCleanup(options = {}) {
  ...
  for (const state of ["done", "failed"]) {
    ...
    let files;
    try { files = await readdir(stateDir); } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to read ${stateDir}: ${e.message}`); continue; }
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (completed && completed < cutoff) {
          ...
          try { await rm(path.join(ARTIFACTS_DIR, taskId), { recursive: true }); } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to remove artifacts for ${taskId}: ${e.message}`); }
          try { await unlink(path.join(LOGS_DIR, `${taskId}.log`)); } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to remove log file for ${taskId}: ${e.message}`); }
          try { await rm(path.join(LOGS_DIR, taskId), { recursive: true }); } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to remove log dir for ${taskId}: ${e.message}`); }
          try { await rm(path.join(WORKSPACES_DIR, taskId), { recursive: true }); } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to remove workspace for ${taskId}: ${e.message}`); }
          cleaned++;
  ...
  for (const orphanId of orphans) {
    ...
    } catch (e) { if (e.code !== "ENOENT") log(`cleanup: failed to remove orphan worktree ${orphanId}: ${e.message}`); }
  }
...
async function loadProjectPreferences(projectPath) {
  try {
    ...
    if (e.code !== "ENOENT") log(`loadProjectPreferences: ${projectPath}: ${e.message}`);
    return "";
  }
}
...
async function runForge(taskId) {
  ...
  wireEvents(fp, (event, data) => {
    ...
    if (event === "stage:gate") {
      updateTaskMeta(taskId, { stageGate: data.stage }).catch((e) => log(`[${taskId}] updateTaskMeta error: ${e.message}`));
    }
    if (event === "stage:gate_resolved") {
      updateTaskMeta(taskId, { stageGate: null }).catch((e) => log(`[${taskId}] updateTaskMeta error: ${e.message}`));
    }
  });
  ...
  try {
    ...
    if (RATE_LIMIT_RE.test(e.message)) {
      try { await moveTask(taskId, "running", "pending"); } catch (moveErr) { log(`[${taskId}] moveTask recovery failed: ${moveErr.message}`); }
      await handleQuotaExceeded();
    } else {
      try { await moveTask(taskId, "running", "failed"); } catch (moveErr) { log(`[${taskId}] moveTask to failed failed: ${moveErr.message}`); }
      daemonState.stats.tasksFailed++;
    }
  } finally {
...
async function requeueSuspendedTasks() {
  ...
  for (const taskId of tasksToRequeue) {
    ...
    if (!currentTask) {
      log(`[${taskId}] requeue: skipped stale suspended entry (task not found)`);
      continue;
    }
    if (currentTask.state === "pending") {
      const alreadyQueuedPending = taskQueueIds.has(taskId) || taskQueue.some((queued) => queued.id === taskId);
      if (!alreadyQueuedPending && !inflightTasks.has(taskId)) {
        taskQueue.push(currentTask);
        taskQueueIds.add(currentTask.id);
        enqueued++;
      }
      log(`[${taskId}] requeue: task already pending${alreadyQueuedPending ? " (already queued)" : ""}`);
      continue;
    }
    if (currentTask.state !== "running" || !currentTask.suspended) {
      log(`[${taskId}] requeue: skipped stale suspended entry (state=${currentTask.state}, suspended=${!!currentTask.suspended})`);
      continue;
    }
    ...
    try {
      ...
      const alreadyQueued = taskQueueIds.has(taskId) || taskQueue.some((queued) => queued.id === taskId);
      ...
      log(`[${taskId}] requeued suspended task → pending${alreadyQueued ? " (already queued)" : ""}`);
    } catch (e) {
      if (e && e.code === "ENOENT") {
        log(`[${taskId}] requeue: skipped stale suspended entry during move (${e.message})`);
        continue;
      }
  ...
  if (enqueued > 0) {
    ...
    log(`requeue: enqueued ${enqueued} suspended task(s) (queue: ${taskQueue.length})`);
    wakeProcessLoop();
  }
  ...
  if (failedTaskIds.length > 0) {
    throw new Error(`failed to requeue ${failedTaskIds.length} suspended task(s): ${failedTaskIds.join(", ")}`);
  }
}
...
async function handleQuotaExceeded() {
  ...
  try { await flushStateNow(); } catch (e) { log(`[warning] failed to persist paused state: ${e.message}`); }
...
async function startDaemon(foreground, devMode, opts = {}) {
  ...
  logStream.on("error", (err) => {
    ...
    try { logStream.end(); } catch (e2) { console.error(`logStream.end() error: ${e2.message}`); }
    logStream = null;
  });
  ...
  ucmdHandlers.setDeps({
    ...
    setProbeTimer: (timer) => { probeTimer = timer; },
  ...
  ucmdServer.setDeps({
    ...
    handlers: () => ({
      ...
      handleProposalDelete, handleProposalPriority, handleProposalEvaluate,
      handleSnapshots,
      handleAnalyzeProject, handleResearchProject,
      performCleanup,
      handleStageGateApprove, handleStageGateReject,
      handleGetConfig, handleSetConfig, handleUpdatePriority,
      handleMergeQueueStatus, handleMergeQueueRetry, handleMergeQueueSkip,
  ...
  const logTruncateTimer = setInterval(() => {
    truncateLogIfNeeded().catch((e) => log(`log truncate error: ${e.message}`));
  }, 60 * 60 * 1000);
...
async function gracefulShutdown() {
  ...
  if (activeForgePipelines.size > 0) {
    ...
    for (const [taskId, fp] of activeForgePipelines) {
      abortPromises.push(
        fp.abort().catch((e) => { log(`abort forge pipeline error for ${taskId}: ${e.message}`); })
      );
  ...
  if (activeChildren.size > 0) {
    ...
    for (const [taskId, child] of activeChildren) {
      try { child.kill("SIGTERM"); } catch (e) { if (e.code !== "ESRCH") log(`failed to SIGTERM agent for ${taskId}: ${e.message}`); }
    ...
    for (const [taskId, child] of activeChildren) {
      try { child.kill("SIGKILL"); } catch (e) { if (e.code !== "ESRCH") log(`failed to SIGKILL agent for ${taskId}: ${e.message}`); }
      log(`sent SIGKILL to agent for ${taskId}`);
    }
  }

  // save merge queue state
  try { await mergeQueueManager.save(); } catch (e) { log(`merge queue save error: ${e.message}`); }

  if (stateDirty) {
    try { await flushState(); } catch (e) { log(`shutdown flush error: ${e.message}`); }
  }

  const { socketSubscribers: ss, socketServer: getSock } = ucmdServer;

  for (const conn of ss) {
    try { conn.end(); } catch (e) { log(`conn.end error during shutdown: ${e.message}`); }
  }
  ss.clear();
  ...
  if (currentSock) {
    currentSock.close();
    try { await unlink(SOCK_PATH); } catch (e) { if (e.code !== "ENOENT") log(`failed to unlink socket: ${e.message}`); }
  }

  try { await unlink(PID_PATH); } catch (e) { if (e.code !== "ENOENT") log(`failed to unlink PID file: ${e.message}`); }
...
function setEmbeddedMode(value) { embeddedMode = value; }
