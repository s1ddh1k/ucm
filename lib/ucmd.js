...
async function createTempWorkspace(taskId) {
  ...
  execFileSync("git", ["commit", "--allow-empty", "-m", "init workspace"], {
    cwd: workspacePath,
    stdio: "pipe",
  });
    ...
    async () => {
      for (const state of TASK_STATES) {
        ...
        try {
          ...
          if (e.code !== "ENOENT")
            console.error(
              `[updateTaskProject] task=${taskId} state=${state} path=${taskPath}: ${e.message}`,
            );
        }
      }
    },
...
async function updateTaskMeta(taskId, updates) {
  const current = enqueueTaskFileOp(
    ...
    async () => {
      for (const state of TASK_STATES) {
        ...
        try {
          ...
          if (e.code !== "ENOENT")
            console.error(
              `[updateTaskMeta] task=${taskId} state=${state} path=${taskPath}: ${e.message}`,
            );
        }
      }
    },
    {
      log,
      label: `updateTaskMeta(stage=${updates?.stageGate || updates?.currentStage || "n/a"})`,
    },
  );
...
async function flushState() {
  ...
  _flushQueue = op.catch((e) => {
    log(`flushState queue error (non-fatal): ${e.message}`);
  });
...
async function flushStateNow() {
  ...
  try {
    ...
    log(`critical state flush error: ${e.message}`);
  }
}
...
async function runForge(taskId) {
  ...
  try {
    ...
    if (status === "merge_queued") {
      ...
      tryAutoApprove(taskId).catch((e) => log(`[${taskId}] tryAutoApprove error: ${e.message}`));
    } else {
...
function stopHmd() {
  ...
  try {
    ...
    if (e.code !== "ESRCH") log(`hmd: failed to SIGTERM (pid: ${pid}): ${e.message}`);
  }
  hmdChild = null;
}
...
async function startDaemon(foreground, devMode, opts = {}) {
  ...
  const observerEnabled = observerConfig.enabled || automationConfig.autoPropose;
  if (observerEnabled) {
