...
function isRetryableIoError(err) {
  return !!(err?.code && RETRYABLE_IO_CODES.has(err.code));
}

function getIoErrorKind(err) {
  ...
  try {
    ...
    if (e && e.code === "ENOENT") {
      ...
      await atomicWriteFile(CONFIG_PATH, `${JSON.stringify(config, null, 2)}\n`, {
        operation: "loadConfig initialize",
        stage: "config-load",
      });
    } else {
...
async function createTempWorkspace(taskId) {
  ...
  await execFileAsync(
    "git",
    ["commit", "--allow-empty", "-m", "init workspace"],
    {
      cwd: workspacePath,
      encoding: "utf-8",
    },
  ...
  try {
    ...
    if (RATE_LIMIT_RE.test(e.message)) {
      ...
      if (movedToPending) {
        ...
        if (pendingTask && !alreadyQueued && !inflightTasks.has(taskId)) {
          taskQueue.push(pendingTask);
          taskQueueIds.add(taskId);
          log(
            `[${taskId}] requeued after rate limit recovery (queue: ${taskQueue.length})`,
          );
        }
      }
...
async function processLoop() {
  while (!shutdownRequested) {
    try {
      ...
      log(
        `processLoop iteration error (daemonStatus=${daemonState.daemonStatus}, queue=${taskQueue.length}, inflight=${inflightTasks.size}): ${e.message}`,
      );
    }
