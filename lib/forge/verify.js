...
async function run({ taskId, dag, project, subtask, timeouts, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[verify] loadArtifact error (spec.md): ${e.message}`);
    try {
      specContent = await loadArtifact(taskId, "task.md");
      onLog("[verify] using fallback artifact: task.md instead of spec.md");
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[verify] loadArtifact error (task.md): ${e2.message}`);
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[verify] loadArtifact error (design${artifactSuffix}.md): ${e.message}`);
    try {
      designContent = await loadArtifact(taskId, "design.md");
      onLog(`[verify] using fallback artifact: design.md`);
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[verify] loadArtifact error (design.md): ${e2.message}`);
...
반드시 JSON만 출력하세요.`;
  ...
  if (testResult.status === "done") {
    try {
      ...
      // JSON parse failure — cannot confirm tests passed, assume failed
      testsPassed = false;
