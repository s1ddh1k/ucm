...
async function run({ taskId, dag, project, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      onLog(`[decompose] loadArtifact error (design.md): ${e.message}`);
