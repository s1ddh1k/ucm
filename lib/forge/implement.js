...
async function run({ taskId, dag, project, autopilot, subtask, feedback, timeouts, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[implement] loadArtifact error (design${artifactSuffix}.md): ${e.message}`);
    try {
      designContent = await loadArtifact(taskId, "design.md");
      onLog(`[implement] using fallback artifact: design.md`);
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[implement] loadArtifact error (design.md): ${e2.message}`);
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[implement] loadArtifact error (spec.md): ${e.message}`);
    try {
      specContent = await loadArtifact(taskId, "task.md");
      onLog("[implement] using fallback artifact: task.md instead of spec.md");
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[implement] loadArtifact error (task.md): ${e2.message}`);
  ...
  } catch (e) { onLog(`[implement] hivemind search skipped: ${e.message}`); }
