...
class ForgePipeline extends EventEmitter {
  ...
  async run() {
    ...
    try {
      ...
      try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after error: ${saveErr.message}` }); }
  ...
  async runIntake() {
    ...
    try {
      ...
      try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after intake error: ${saveErr.message}` }); }
      this.emit("stage:complete", { stage: "intake", durationMs, status: "fail" });
      throw error;
    }
  }
  ...
  async checkRequiredArtifacts(stageName) {
    ...
    for (const artifact of stageArtifacts.requires) {
      ...
      try {
        ...
        if (e.code !== "ENOENT") this.emit("agent:output", { stage: stageName, chunk: `[forge] loadArtifact error (${artifact}): ${e.message}` });
  ...
  async runStage(stageName, { subtask } = {}) {
    ...
    try {
      ...
      try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after ${stageName} error: ${saveErr.message}` }); }
    ...
    for (const wave of waves) {
      ...
      for (const subtaskId of pending) {
        ...
        try {
          ...
          try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after subtask ${subtaskId} error: ${saveErr.message}` }); }
  ...
  async runImplementVerifyLoop(subtask) {
    ...
    if (this.resumeFrom === "implement") {
      try {
        ...
        if (e.code !== "ENOENT") this.emit("agent:output", { stage: "implement", chunk: `[forge] loadArtifact error (rejection-feedback.md): ${e.message}` });
      }
    }
    ...
    if (this.resumeFrom && !feedback) {
      ...
      for (let i = maxIterations; i >= 1; i--) {
        try {
          const raw = await loadArtifact(this.taskId, `verify-iter-${i}${subtaskSuffix}.json`);
          const data = JSON.parse(raw);
          if (data.feedback && !data.passed) {
            feedback = data.feedback;
            this.emit("agent:output", { stage: "implement", chunk: `[forge] loaded previous verify feedback from iteration ${i}` });
            break;
          }
        } catch {
    ...
    for (let iteration = 1; iteration <= maxIterations; iteration++) {
      ...
      try {
        ...
        try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after implement error: ${saveErr.message}` }); }
      ...
      try {
        ...
        try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after verify error: ${saveErr.message}` }); }
        this.emit("stage:complete", { stage: "verify", durationMs: verifyDuration, status: "fail" });
        this.emit("agent:output", { stage: "verify", chunk: `[verify] crashed: ${verifyError.message}` });
    ...
    try { await this.dag.save(); } catch (saveErr) { this.emit("agent:output", { stage: "save", chunk: `[forge] failed to save DAG after verify exhaustion: ${saveErr.message}` }); }
    throw new Error(msg);
  }
  ...
  async learnToHivemind() {
    try {
      ...
      let summary = "";
      try { summary = await loadArtifact(this.taskId, "summary.md"); } catch (e) {
        if (e.code !== "ENOENT") this.emit("agent:output", { stage: "hivemind", chunk: `[forge] loadArtifact error (summary.md): ${e.message}` });
        this.emit("agent:output", { stage: "hivemind", chunk: "[forge] no summary artifact available for hivemind" });
      }
  ...
  async abort() {
    ...
    try {
      ...
      this.emit("agent:output", { stage: "abort", chunk: `[forge] worktree cleanup on abort failed: ${e.message}` });
    }
    ...
    if (this.dag) {
      ...
      try { await this.dag.save(); } catch (e) { this.emit("agent:output", { stage: "abort", chunk: `[forge] failed to save DAG on abort: ${e.message}` }); }
    }
  }
}
