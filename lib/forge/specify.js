...
async function run({ taskId, dag, project, timeouts, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[specify] loadArtifact error (decisions.json): ${e.message}`);
    try {
      decisionsContent = await loadArtifact(taskId, "decisions.md");
      decisions = parseDecisionsFile(decisionsContent);
      onLog("[specify] using fallback artifact: decisions.md instead of decisions.json");
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[specify] loadArtifact error (decisions.md): ${e2.message}`);
  ...
  if (decisions.length === 0) {
    try {
      ...
      if (e.code !== "ENOENT") onLog(`[specify] loadArtifact error (task.md): ${e.message}`);
...
${decisionsText}`;
  ...
  for (const id of parallelResult.done) {
    ...
    } catch (e) { if (e.code !== "ENOENT") onLog(`[specify] failed to read worker output ${id}: ${e.message}`); }
  }
