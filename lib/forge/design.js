...
async function run({ taskId, dag, project, subtask, timeouts, onLog = () => {} } = {}) {
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[design] loadArtifact error (spec.md): ${e.message}`);
    try {
      specContent = await loadArtifact(taskId, "task.md");
      onLog("[design] using fallback artifact: task.md instead of spec.md");
    } catch (e2) {
      if (e2.code !== "ENOENT") onLog(`[design] loadArtifact error (task.md): ${e2.message}`);
  ...
  try {
    ...
    if (e.code !== "ENOENT") onLog(`[design] loadArtifact error (decisions.json): ${e.message}`);
