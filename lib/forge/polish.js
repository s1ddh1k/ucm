...
반드시 JSON만 출력하세요.`;
  ...
  if (result.status !== "done") {
    onLog(`[polish] test gate agent ${result.status} (not a test failure) — skipping gate for round ${round}`);
    return { passed: true, skipped: true, failures: [], tokenUsage: result.tokenUsage };
  }
