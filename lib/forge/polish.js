...
async function run({
  ...
  try {
    ...
    if (e.code !== "ENOENT")
      onLog(
        `[polish] loadArtifact error (design${artifactSuffix}.md): ${e.message}`,
      );
    try {
      ...
      if (e2.code !== "ENOENT")
        onLog(`[polish] loadArtifact error (design.md): ${e2.message}`);
  ...
  for (const lensName of config.defaultLenses) {
    if (totalRounds >= config.maxTotalRounds) {
      onLog(
        `[polish] max total rounds (${config.maxTotalRounds}) reached, skipping remaining lenses`,
      );
      break;
    }
  ...
  // 미해결 major 이슈 수집: converge하지 못한 렌즈의 마지막 라운드에서 major 이슈 추출
  const unresolvedIssues = [];
  for (const lr of lensResults) {
    if (lr.converged) continue;
    try {
      const lastRoundArtifact = await loadArtifact(
        taskId,
        `polish-${lr.lens}-round-${lr.rounds}${artifactSuffix}.json`,
      );
      const lastRound = JSON.parse(lastRoundArtifact);
      const majors = (lastRound.issues || []).filter((i) => i.severity === "major");
      for (const issue of majors) {
        unresolvedIssues.push({ lens: lr.lens, ...issue });
      }
    } catch (e) {
      if (e.code !== "ENOENT") {
        onLog(`[polish] failed to load round artifact for ${lr.lens}: ${e.message}`);
      }
    }
  }
  ...
  if (hasUnresolvedIssues) {
    feedback = `## Polish 미해결 이슈 (${unresolvedIssues.length}건)\n\n` +
      unresolvedIssues.map((issue, i) =>
        `${i + 1}. [${issue.lens}] ${issue.file || ""}${issue.line ? `:${issue.line}` : ""}\n   ${issue.description}\n   제안: ${issue.suggestion || "없음"}`
      ).join("\n\n");
    onLog(`[polish] ${unresolvedIssues.length} unresolved major issues — triggering feedback iteration`);
  }

  await saveArtifact(
    taskId,
    `polish-summary${artifactSuffix}.json`,
    JSON.stringify(summary, null, 2),
  );

  onLog(
    `[polish] complete: ${totalRounds} rounds, ${totalIssuesFound} issues found across ${lensResults.length} lenses`,
  );

  return { status: "pass", summary, tokenUsage: totalTokenUsage, hasUnresolvedIssues, unresolvedIssues, feedback };
}

async function runLensLoop({
  lensName,
  taskId,
  dag,
  project,
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    await saveArtifact(
      ...
      JSON.stringify(
        { lens: lensName, round, issues, summary: reviewResult.summary || "" },
        null,
        2,
      ),
    );
    ...
    if (issues.length === 0) {
      consecutiveClean++;
      onLog(
        `[polish:${lensName}] round ${round} — 0 issues (clean ${consecutiveClean}/${config.convergenceThreshold})`,
      );
    ...
    onLog(
      `[polish:${lensName}] round ${round} — ${issues.length} issues found, fixing...`,
    );
    ...
    if (!testResult.passed) {
      onLog(
        `[polish:${lensName}] round ${round} — test gate FAILED, fixing tests...`,
      );
...
반드시 JSON만 출력하세요.`;
  ...
  // 단, 반드시 로그를 남겨 투명성을 확보한다.
  if (result.status !== "done") {
    onLog(
      `[polish] test gate agent ${result.status} (not a test failure) — skipping gate for round ${round}`,
    );
    return {
      passed: true,
      skipped: true,
