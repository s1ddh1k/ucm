...
const { mkdir, access, unlink } = require("node:fs/promises");
...
async function validate(opts) {
  ...
  if (opts.specTemplate) {
    try {
      await access(path.resolve(opts.specTemplate));
    } catch {
      errors.push(`spec 템플릿 파일 없음: ${opts.specTemplate}`);
    }
  }
  if (opts.project) {
    try {
  ...
  if (
    ...
    (Number.isNaN(opts.maxRounds) || opts.maxRounds < 1)
  ) {
...
function spawnTool(scriptPath, args, { interactive }) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code !== 0)
        return reject(new Error(`${path.basename(scriptPath)} exit ${code}`));
      resolve(out.trim());
    });
...
function buildQnaArgs({
  ...
  if (round > 0) {
    ...
    if (feedbackFilePath)
      qnaArgs.push("--feedback-file", path.resolve(feedbackFilePath));
  }
...
async function main() {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    // follow-up qna에서 이전 라운드 gap-report를 읽은 뒤 정리해야 한다.
    try {
      await unlink(gapReportPath);
    } catch {}
    ...
    if (opts.specTemplate)
      specArgs.push("--template", path.resolve(opts.specTemplate));
    ...
    try {
      requirementsPath = await spawnTool(specPath, specArgs, {
        interactive: false,
      });
    } catch (e) {
  ...
  if (!converged) {
    process.stderr.write(
      `요구사항 검증이 최대 라운드 내 수렴하지 않았습니다. 마지막 요구사항 결과를 실패로 처리합니다.\n`,
    );
    process.exit(1);
  }
