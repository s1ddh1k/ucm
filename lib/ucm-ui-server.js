...
const PROXY_ROUTES = [
  ...
  // Proposal scoring
  {
    ...
    params: (url, m) => ({ proposalId: m[1], recompute: url.searchParams.get("recompute") === "1" }),
  },
  {
    pattern: /^\/api\/proposal\/score\/(p-[a-f0-9]+)$/,
    post: true,
  ...
  // Clustering
  {
    ...
    params: (url) => ({ status: url.searchParams.get("status") || undefined, refresh: url.searchParams.get("refresh") === "1" }),
  },
  {
