...
const PROXY_ROUTES = [
  ...
  {
    pattern: /^\/api\/autopilot\/session\/([a-z0-9_]+)$/,
    method: "autopilot_session",
    params: (_, m) => ({ sessionId: m[1] }),
  },
  {
    pattern: /^\/api\/autopilot\/start$/,
    post: true,
    method: "autopilot_start",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/pause$/,
    post: true,
    method: "autopilot_pause",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/resume$/,
    post: true,
    method: "autopilot_resume",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/stop$/,
    post: true,
    method: "autopilot_stop",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/approve-item$/,
    post: true,
    method: "autopilot_approve_item",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/reject-item$/,
    post: true,
    method: "autopilot_reject_item",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/feedback-item$/,
    post: true,
    method: "autopilot_feedback_item",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/releases\/([a-z0-9_]+)$/,
    method: "autopilot_releases",
    params: (_, m) => ({ sessionId: m[1] }),
  },
  // Autopilot directive routes
  {
    pattern: /^\/api\/autopilot\/directives\/([a-z0-9_]+)$/,
    method: "autopilot_directive_list",
    params: (url, m) => ({
      sessionId: m[1],
  ...
  {
    pattern: /^\/api\/autopilot\/directive\/add$/,
    post: true,
    method: "autopilot_directive_add",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/directive\/edit$/,
    post: true,
    method: "autopilot_directive_edit",
    bodyParams: true,
  },
  {
    pattern: /^\/api\/autopilot\/directive\/delete$/,
    post: true,
    method: "autopilot_directive_delete",
    bodyParams: true,
  },
];
...
const WS_FORWARD_ACTIONS = new Set([
  ...
  "autopilot_resume",
  "autopilot_stop",
  "autopilot_approve_item",
  "autopilot_reject_item",
  "autopilot_feedback_item",
  "autopilot_directive_add",
  "autopilot_directive_edit",
  "autopilot_directive_delete",
]);
...
function startSessionWatcher(projectDir, onNewSession) {
  const known = new Set();
  try {
  ...
  buildSystemPrompt(resolvedCwd)
    .then(async (systemPrompt) => {
      ...
      if (providerConfig.supportsWatcher && ptySessionId) {
        ...
        setTimeout(() => {
          const projectDir = discoverClaudeProjectDir(
            resolvedCwd,
            capturedSessionId,
          );
          if (projectDir) {
            const watcher = startSessionWatcher(projectDir, (newId) => {
...
function resolvePathWithinHome(inputPath) {
  const resolved = path.resolve(resolveHomePath(inputPath));
  const home = os.homedir();
  return { resolved, allowed: isWithinDirectory(resolved, home), home };
}
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname === "/api/browse") {
        ...
        const { resolved, allowed } = resolvePathWithinHome(requestedPath);
        if (!allowed) {
          jsonResponse(
            res,
      ...
      if (req.method === "POST" && pathname === "/api/mkdir") {
        ...
        const { resolved, allowed } = resolvePathWithinHome(dirPath);
        if (!allowed) {
          jsonResponse(
            res,
            { error: "access denied: path must be within home directory" },
