...
const PROXY_ROUTES = [
  ...
  { pattern: new RegExp(`^/api/stage-gate/reject/${TASK_ID_PARAM}$`), post: true, method: "stage_gate_reject",
    params: (_, m, body) => ({ taskId: m[1], ...body }) },
  // Merge queue routes
  { pattern: /^\/api\/merge-queue$/, method: "merge_queue_status",
    params: (url) => ({ project: url.searchParams.get("project") || undefined }) },
  { pattern: new RegExp(`^/api/merge-queue/retry/${TASK_ID_PARAM}$`), post: true, method: "merge_queue_retry",
    params: (_, m) => ({ taskId: m[1] }) },
  { pattern: new RegExp(`^/api/merge-queue/skip/${TASK_ID_PARAM}$`), post: true, method: "merge_queue_skip",
    params: (_, m) => ({ taskId: m[1] }) },
  // Config routes
