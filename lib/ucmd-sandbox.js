...
function discardChanges(cwd = SOURCE_ROOT) {
  try { gitSync(["checkout", "--", "."], cwd); } catch (e) { log(`discardChanges: checkout failed: ${e.message}`); }
  try { gitSync(["clean", "-fd"], cwd); } catch (e) { log(`discardChanges: clean failed: ${e.message}`); }
}
...
function detectTestCommand(projectPath) {
  ...
  try {
    ...
    if (e.code && e.code !== "ENOENT") log(`detectTestCommand: package.json read error: ${e.message}`);
  }
  ...
  try {
    ...
    if (e.code && e.code !== "ENOENT") log(`detectTestCommand: Makefile read error: ${e.message}`);
  }
  ...
  try {
    ...
    if (e.code && e.code !== "ENOENT") log(`detectTestCommand: pytest.ini stat error: ${e.message}`);
  }
  try {
    fs.statSync(path.join(projectPath, "setup.py"));
    return { type: "pytest", command: ["python", "-m", "pytest"], cwd: projectPath };
  } catch (e) {
    if (e.code && e.code !== "ENOENT") log(`detectTestCommand: setup.py stat error: ${e.message}`);
  }
