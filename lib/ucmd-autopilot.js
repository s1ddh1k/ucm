...
async function deleteSessionFile(sessionId) {
  try { await unlink(path.join(AUTOPILOT_DIR, `${sessionId}.json`)); } catch (e) {
    if (e.code !== "ENOENT") log(`[autopilot] deleteSessionFile ${sessionId} error: ${e.message}`);
  }
}
...
async function executeItem(session, item) {
  ...
  try {
    ...
    try { await pipeline.abort(); } catch (abortErr) {
      addLog(session, `abort after forge error failed: ${abortErr.message}`, "error");
    }
    return "failed";
  }
}
...
async function performRelease(session) {
  ...
  try {
    ...
    try {
      ...
      log(`[autopilot] generateProjectContext failed for ${session.project}: ${e.message}`);
    }
...
function handleAutopilotResume(params = {}) {
  ...
  if (session._reviewResolve && !session._reviewTimer) {
    ...
    const reviewTimeoutMs = ap.reviewTimeoutMs || DEFAULT_CONFIG.autopilot.reviewTimeoutMs;
    ...
    session._reviewTimer = setTimeout(() => {
      if (session._reviewResolve === reviewResolve) {
        ...
        addLog(session, `review timed out after ${mins} minutes (post-resume), auto-approving`, "warn");
