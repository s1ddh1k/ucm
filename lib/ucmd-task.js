...
function serializeTaskFile(meta, body) {
  ...
  for (const [key, value] of Object.entries(meta)) {
    ...
    if (Array.isArray(value)) {
      const hasComplexValues = value.some((item) => item && typeof item === "object");
      if (hasComplexValues) {
...
function normalizeProjects(meta) {
  ...
  function toProject(entry, index) {
    ...
    if (typeof entry === "string") {
      ...
      origin = typeof entry.origin === "string" ? path.resolve(expandHome(entry.origin)) : null;
      baseCommit = typeof entry.baseCommit === "string" ? entry.baseCommit : null;
    } else {
      return null;
    }
  ...
  if (Array.isArray(meta.projects)) {
    meta.projects.forEach((project, index) => append(project, index));
