  ...
  switch (method) {
    ...
    case "resume": {
      ...
      const fp = new ForgePipeline({
        ...
        tokenBudget: params.tokenBudget,
        resumeFrom:
          params.fromStage || lastFailedStage(existingDag) || "implement",
      });
      activePipelines.set(fp.taskId, fp);
      wireEvents(fp, createForgeEventHandler());
      fp.run()
    ...
    case "abort": {
      ...
      if (running) {
        ...
        try {
          ...
          if (workspace)
            await removeWorktrees(params.taskId, workspace.projects);
        } catch (e) {
          console.error(
            `[forge-server] abort worktree cleanup failed for ${params.taskId}: ${e.message}`,
          );
          dag.warnings.push(`worktree cleanup failed on abort: ${e.message}`);
        }
        dag.status = "aborted";
    ...
    case "logs": {
      ...
      try {
        const limit = normalizeLogLineLimit(params.lines);
        return await readLogTail(logPath, limit);
      } catch (error) {
        ...
        throw new Error(
          `failed to read logs for ${params.taskId}: ${detail}. Check file permissions for ${logPath} and verify task state.`,
        );
      }
    }
...
async function detectOrphanTasks() {
  try {
    ...
    console.error(`[forge-server] detectOrphanTasks failed: ${e.message}`);
  }
}
