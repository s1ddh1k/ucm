const { readFile, writeFile, readdir, unlink, rename } = require("fs/promises");
...
function setDeps(d) { deps = { log: () => {}, broadcastWs: () => {}, ...d }; }
  ...
  for (const status of PROPOSAL_STATUSES) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        log(`[observer] getExistingDedupHashes: skipping ${file}: ${e.message}`);
      }
    }
  }
  return hashes;
...
async function collectObservationData() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (meta.completedAt && meta.completedAt >= cutoff) {
          ...
          } catch (e) { if (e.code !== 'ENOENT') log(`[observer] memory.json read error for ${taskId}: ${e.message}`); }
        ...
        log(`[observer] collectObservationData: skipping task file ${file}: ${e.message}`);
      }
    }
  }
  ...
  try {
    ...
    for (const projectDir of projectDirs) {
      ...
      for (const file of files) {
        ...
        } catch (e) { if (e.code !== 'ENOENT') log(`[observer] lesson read error ${file}: ${e.message}`); }
      }
    }
  } catch (e) {
  ...
  try {
    ...
    for (const file of templateFiles) {
      ...
      } catch (e) { if (e.code !== 'ENOENT') log(`[observer] template read error ${file}: ${e.message}`); }
    }
  } catch (e) { if (e.code !== 'ENOENT') log(`[observer] templates readdir error: ${e.message}`); }
  ...
  for (const projectPath of uniqueProjectPaths) {
    try {
      ...
      if (e.code !== "ENOENT") log(`[observer] scanProjectStructure error for ${projectPath}: ${e.message}`);
  ...
  for (const projectPath of uniqueProjectPaths) {
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[observer] analyzeCommitHistory error for ${name}: ${e.message}`);
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[observer] scanDocumentation error for ${name}: ${e.message}`);
  ...
  try {
    ...
    log(`[observer] collectObservationData: hivemind query failed: ${e.message}`);
  }
  ...
  for (const file of implFiles) {
    ...
    } catch (e) { if (e.code !== 'ENOENT') log(`[observer] evaluation history read error ${file}: ${e.message}`); }
  }
...
async function requestLlmCurationDecision({ proposals, source, maxKeep, maxPerProject, anchorProposal }) {
  ...
  const config = deps.config();
  const observerConfig = config?.observer || DEFAULT_CONFIG.observer;
  const curation = observerConfig.curation || DEFAULT_CONFIG.observer.curation || {};
  const maxInput = Number(curation.maxInputProposals || 80);
  const inputProposals = proposals.slice(0, maxInput).map(compactProposalForCuration);

  const promptParts = [];
  promptParts.push("You are an expert proposal curator for an autonomous software factory.");
  promptParts.push("Goal: keep only high-signal proposals, remove semantic duplicates, and remove mutually exclusive conflicts.");
  if (anchorProposal) {
    promptParts.push("Special rule: the anchor proposal is already approved. Reject any proposal that conflicts with or duplicates the anchor.");
    promptParts.push(`Anchor JSON:\n${JSON.stringify(compactProposalForCuration(anchorProposal), null, 2)}`);
  }
  promptParts.push(`Source: ${source}`);
  if (maxKeep && Number(maxKeep) > 0) promptParts.push(`Maximum total kept proposals: ${Number(maxKeep)}`);
  if (maxPerProject && Number(maxPerProject) > 0) promptParts.push(`Maximum kept proposals per project: ${Number(maxPerProject)}`);
  ...
  try {
    const result = await deps.spawnAgent(promptParts.join("\n\n"), {
      ...
      timeoutMs: curation.timeoutMs || config.stageTimeoutMs || DEFAULT_CONFIG.stageTimeoutMs,
...
async function curateProposalsWithLlm(proposals, { source, maxKeep = 0, maxPerProject = 0, anchorProposal = null } = {}) {
  ...
  if (Array.isArray(decision.keepIds) && decision.keepIds.length > 0) {
    ...
    keptIds = new Set([...idToProposal.keys()].filter((id) => !dropReason.has(id)));
  }
...
async function curateProposedBacklog(source = "observer") {
  const config = deps.config();
  const curation = config?.observer?.curation || DEFAULT_CONFIG.observer.curation || {};
  ...
  if (rejected > 0) {
    log(`[curation] ${source}: curated proposed backlog ${proposed.length} → ${kept.length}`);
  }
  return { total: proposed.length, kept: kept.length, rejected };
}

async function rejectConflictingProposalsForAnchor(anchorProposal, source = "approval") {
  const config = deps.config();
  const curation = config?.observer?.curation || DEFAULT_CONFIG.observer.curation || {};
  if (curation.enabled === false || curation.autoRejectOnApprove === false) return 0;
  ...
  for (const item of decision.dropped) {
    const reason = item.reason || `conflicts with approved ${anchorProposal.id}`;
    if (await rejectProposedProposal(item.proposal, reason, source)) {
      rejected++;
    }
  }
  if (rejected > 0) {
    log(`[curation] ${source}: auto-rejected ${rejected} conflicting proposal(s)`);
  }
  return rejected;
}
...
async function regulateProposal(proposal, existingProposals) {
  ...
  if (regulatorConfig.blockRecentlyFailed) {
    ...
    for (const file of rejectedFiles) {
      ...
      } catch (e) { if (e.code !== 'ENOENT') log(`[observer] dedup rejected read error ${file}: ${e.message}`); }
    }
    ...
    for (const file of implementedFiles) {
      ...
      } catch (e) { if (e.code !== 'ENOENT') log(`[observer] dedup implemented read error ${file}: ${e.message}`); }
    }
  }
...
async function runObserver() {
  ...
  try {
    ...
    const observerConfig = config?.observer || DEFAULT_CONFIG.observer;
    const curationConfig = observerConfig.curation || DEFAULT_CONFIG.observer.curation || {};
    ...
    const curated = curationConfig.enabled === false
...
async function promoteProposal(proposalId) {
  ...
  } catch (e) { log(`[observer] failed to update promoted proposal ${proposalId}: ${e.message}`); }
...
async function handleProposalApprove(params) {
  ...
  deps.broadcastWs("proposal:updated", { id: proposalId, proposalId, status: "approved" });
...
async function handleProposalReject(params) {
  ...
  deps.broadcastWs("proposal:updated", { id: proposalId, proposalId, status: "rejected" });
...
async function handleProposalDelete(params) {
  ...
  deps.broadcastWs("proposal:deleted", { id: proposalId, proposalId, status: deleted.previousStatus });
...
async function cleanupOldProposals() {
  ...
  for (const file of files) {
    ...
    } catch (e) { if (e.code !== 'ENOENT') log(`[observer] cleanup rejected proposal error ${file}: ${e.message}`); }
  }
}
...
async function analyzeProject(projectPath, projectName) {
  ...
  try {
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[analyze] scanProjectStructure error for ${projectName}: ${e.message}`);
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[analyze] analyzeCommitHistory error for ${projectName}: ${e.message}`);
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[analyze] scanDocumentation error for ${projectName}: ${e.message}`);
    ...
    for (const state of ["done", "failed"]) {
      ...
      for (const file of files) {
        ...
        } catch (e) { if (e.code !== 'ENOENT') log(`[observer] task file read error ${file}: ${e.message}`); }
      }
    }
    ...
    } catch (e) { if (e.code !== 'ENOENT') log(`[observer] lessons readdir error: ${e.message}`); }
    ...
    } catch (e) { log(`[observer] hivemind knowledge query error: ${e.message}`); }
...
async function runResearch(projectPath, projectName) {
  ...
  try {
    ...
    try {
      ...
      if (e.code !== "ENOENT") log(`[research] scanProjectStructure error for ${projectName}: ${e.message}`);
      codeStructure[projectName] = { path: projectPath, error: "inaccessible" };
    }
    ...
    } catch (e) { log(`[observer] doc coverage scan error for ${projectName}: ${e.message}`); }
    ...
    const existingHashes = await getExistingDedupHashes();
    const curation = (config?.observer || DEFAULT_CONFIG.observer).curation || DEFAULT_CONFIG.observer.curation || {};
    const curated = curation.enabled === false
...
module.exports = {
  ...
  curateProposedBacklog, rejectConflictingProposalsForAnchor, curateProposalsWithLlm,
