...
const curation = require("./ucmd-curation.js");
...
async function getExistingDedupHashes() {
  ...
  // Also include discarded proposal hashes to prevent re-generation
  try {
    const discardedHashes = await curation.getDiscardedDedupHashes();
    for (const h of discardedHashes) hashes.add(h);
  } catch (e) {
    log(`[observer] getExistingDedupHashes: discard history error: ${e.message}`);
  }
  return hashes;
}
...
async function validateProposalQuality(proposal) {
  let config;
  try {
    config = deps.config ? deps.config() : {};
  } catch (e) {
    log(`[observer] quality gate config error (fail-open): ${e.message}`);
    return { passed: true, error: e.message };
  }
  const gate = config?.observer?.qualityGate ?? DEFAULT_CONFIG.observer.qualityGate;
  if (!gate.enabled) return { passed: true };

  const prompt = `아래 제안의 품질을 4가지 차원에서 1-5점으로 평가하세요.

## 제안
- 제목: ${proposal.title}
- 카테고리: ${proposal.category || "unknown"}
- 리스크: ${proposal.risk || "unknown"}
- 설명: ${proposal.description || ""}
${proposal.rationale ? `- 근거: ${proposal.rationale}` : ""}

## 평가 차원
1. **clarity** — 제안이 무엇을 변경하는지 명확한가
2. **feasibility** — 기술적으로 실현 가능한가
3. **completeness** — 구현에 필요한 정보가 충분한가
4. **consistency** — 기존 아키텍처/패턴과 일관적인가

## 응답 형식 (JSON만 출력)
{
  "scores": { "clarity": 1-5, "feasibility": 1-5, "completeness": 1-5, "consistency": 1-5 },
  "reason": "평가 근거 요약 (1-2문장)"
}`;

  try {
    const { data: result, tokenUsage } = await llmJson(prompt, {
      model: gate.model || "sonnet",
      hardTimeoutMs: gate.timeoutMs || 60000,
    });

    const scores = result.scores || {};
    const dims = ["clarity", "feasibility", "completeness", "consistency"];
    const values = dims.map((d) => Number(scores[d]) || 0);
    const avg = values.reduce((a, b) => a + b, 0) / values.length;
    const minScore = gate.minScore ?? 3;
    const passed = avg >= minScore;

    if (!passed) {
      log(`[observer] quality gate rejected: ${proposal.title} — avg=${avg.toFixed(1)} < ${minScore} (${result.reason || ""})`);
    }

    return { passed, scores, avg, reason: result.reason || "", tokenUsage };
  } catch (e) {
    // fail-open: LLM 호출 실패 시 통과 처리
    log(`[observer] quality gate error (fail-open): ${e.message}`);
    return { passed: true, error: e.message };
  }
}
...
async function runObserver() {
  ...
  try {
    ...
    for (const proposal of allProposals) {
      ...
      const quality = await validateProposalQuality(proposal);
    ...
    // autoConvert: auto-approve and promote proposals to tasks
    const { resolveAutomation } = require("./ucmd-handlers.js");
    for (const proposal of savedProposals) {
      const projectPath = proposal.project || null;
      const toggles = resolveAutomation(projectPath);
      if (toggles.autoConvert) {
        try {
          await moveProposal(proposal.id, "proposed", "approved");
          deps.broadcastWs("proposal:updated", { id: proposal.id, proposalId: proposal.id, status: "approved" });
          await promoteProposal(proposal.id);
          log(`[observer] autoConvert: promoted ${proposal.id} → task`);
        } catch (e) {
          log(`[observer] autoConvert failed for ${proposal.id}: ${e.message}`);
        }
      }
    }
...
async function handleProposalApprove(params) {
  ...
  const proposal = await loadProposal(proposalId);
  if (!proposal) throw new Error(`proposal not found: ${proposalId}`);
  const approvableStatuses = ["proposed", "packaged"];
  if (!approvableStatuses.includes(proposal.status))
    throw new Error(`proposal cannot be approved from state: ${proposal.status}`);

  await moveProposal(proposalId, proposal.status, "approved");
  deps.broadcastWs("proposal:updated", {
    id: proposalId,
    proposalId,
    status: "approved",
...
async function handleProposalReject(params) {
  ...
  const proposal = await loadProposal(proposalId);
  if (!proposal) throw new Error(`proposal not found: ${proposalId}`);
  const rejectableStatuses = ["proposed", "packaged"];
  if (!rejectableStatuses.includes(proposal.status))
    throw new Error(`proposal cannot be rejected from state: ${proposal.status}`);

  await moveProposal(proposalId, proposal.status, "rejected");
  deps.broadcastWs("proposal:updated", {
    id: proposalId,
    proposalId,
    status: "rejected",
...
function maybeRunObserver() {
  ...
  if (!observerConfig.enabled && !automationConfig.autoPropose) return;
...
async function handleCurationMode() {
  const modeData = await curation.loadCurationMode();
  const config = deps.config();
  const latestSnapshot = await loadLatestSnapshot();
  ...
  if (modeData.mode === "stabilization") {
    transitionScore = curation.checkTransitionToBigBet(metrics, modeData, config);
  } else {
    transitionScore = curation.checkTransitionToStabilization(metrics, config);
  }

  return {
    ...modeData,
    transitionScore,
    config: config?.curation?.transitionThresholds || {},
  };
}

async function handleSetCurationMode(params) {
  const { mode, reason } = params;
  if (!mode) throw new Error("mode required");
  return curation.setMode(mode, reason);
}

async function handleCurationWeights(params) {
  const config = deps.config();
  if (params?.weights) {
    // Persist custom weights for the active profile via config
    const activeProfile =
      config?.curation?.scoring?.activeProfile || "default";
    const currentProfile = curation.getWeightProfile(activeProfile, config);
    const newWeights = { ...currentProfile.weights, ...params.weights };
    const h = deps.handlers();
    if (h?.handleSetConfig) {
      const profiles = { ...(config?.curation?.scoring?.profiles || {}) };
      profiles[activeProfile] = { ...profiles[activeProfile], weights: newWeights };
      await h.handleSetConfig({ curation: { scoring: { profiles } } });
    }
    return { activeProfile, weights: newWeights };
  }
  if (params?.profile) {
    // Persist profile switch via config
    const h = deps.handlers();
    if (h?.handleSetConfig) {
      await h.handleSetConfig({ curation: { scoring: { activeProfile: params.profile } } });
    }
    const updatedConfig = deps.config();
    return {
      activeProfile: params.profile,
      profiles: curation.listScoringProfiles(updatedConfig),
    };
  }
  return {
    activeProfile:
      config?.curation?.scoring?.activeProfile || "default",
    profiles: curation.listScoringProfiles(config),
  };
}

async function handleProposalScore(params) {
  const { proposalId, recompute } = params;
  if (!proposalId) throw new Error("proposalId required");
  const proposal = await loadProposal(proposalId);
  if (!proposal) throw new Error(`proposal not found: ${proposalId}`);

  if (recompute && proposal.scores) {
    const config = deps.config();
    proposal.priority = curation.recalcPriority(proposal, config);
    await saveProposal(proposal);
  }
  ...
  return {
    ...
    priority: proposal.priority || 0,
    scoreSource: proposal.scoreSource || null,
    weightProfile: proposal.weightProfile || null,
  };
}

async function handleProposalScoreSet(params) {
  const { proposalId, scores } = params;
  if (!proposalId) throw new Error("proposalId required");
  if (!scores) throw new Error("scores required");

  const proposal = await loadProposal(proposalId);
  if (!proposal) throw new Error(`proposal not found: ${proposalId}`);

  const previousScores = proposal.scores || null;
  proposal.scores = { ...proposal.scores, ...scores };
  proposal.scoreSource = proposal.scoreSource === "ai" ? "mixed" : "user";

  const config = deps.config();
  proposal.priority = curation.recalcPriority(proposal, config);
  await saveProposal(proposal);

  await curation.recordCurationAction(
    proposalId,
    "score_adjust",
    previousScores,
    proposal.scores,
  ...
  deps.broadcastWs("proposal:scored", {
    proposalId,
    scores: proposal.scores,
    weightedRank: proposal.priority,
  });
...
async function handleProposalClusters(params) {
  if (params?.refresh) {
    const config = deps.config();
    return curation.clusterProposals(config);
  }
  return curation.loadClusters();
}

async function handleProposalClusterMerge(params) {
  const { proposalIds, representativeId } = params;
  if (!proposalIds || !Array.isArray(proposalIds) || proposalIds.length < 2) {
    throw new Error("proposalIds must be an array of at least 2 IDs");
  }
  return curation.mergeIntoClusters(proposalIds, representativeId);
}

async function handleProposalClusterSplit(params) {
  const { proposalId } = params;
  if (!proposalId) throw new Error("proposalId required");
  return curation.splitFromCluster(proposalId);
}

async function handleProposalConflicts(params) {
  const { proposalId } = params;
  if (!proposalId) throw new Error("proposalId required");
  return curation.detectConflicts(proposalId);
}

async function handleProposalDiscard(params) {
  const { proposalId, reason, discardedBy } = params;
  if (!proposalId) throw new Error("proposalId required");
  return curation.discardProposal(
    proposalId,
    reason || "manual",
    discardedBy || "user",
  );
...
async function handleDiscardHistory(params) {
  const history = await curation.loadDiscardHistory();
  let records = history.records;

  if (params?.project) {
    records = records.filter(
      (r) => r.project === params.project || r.category === params.project,
    );
  }
  if (params?.reason) {
    records = records.filter((r) => r.reason === params.reason);
  }

  const limit = params?.limit || 50;
  records = records.slice(-limit);

  return { records, total: history.records.length };
}

async function handleBigBetChecklist(params) {
  const { proposalId, recompute } = params;
  if (!proposalId) throw new Error("proposalId required");
  return curation.computeReadinessChecklist(proposalId);
}

async function handleProposalFeedback(params) {
  const { proposalId, taskId, outcome } = params;
  if (!proposalId) throw new Error("proposalId required");
  if (!taskId) throw new Error("taskId required");
  return curation.recordFeedback(proposalId, taskId, outcome || {});
}

async function handleScoringProfile(params) {
  const config = deps.config();
  return curation.listScoringProfiles(config);
}
