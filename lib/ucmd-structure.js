...
async function scanProjectStructure(projectPath, { topN = 10 } = {}) {
  ...
  async function walk(dir) {
    let entries;
    try { entries = await readdir(dir, { withFileTypes: true }); } catch (err) { if (err.code !== "ENOENT") console.error(`[structure] readdir failed: ${dir}: ${err.message}`); return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        } catch (err) { if (err.code !== "ENOENT") console.error(`[structure] analyzeFile failed: ${fullPath}: ${err.message}`); }
      }
    }
  }
  ...
  for (const commit of commits) {
    ...
    try {
      const stat = execFileSync(
        "git", ["show", "--shortstat", "--format=", commit.hash],
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && DOC_DIRS.has(entry.name.toLowerCase())) {
        ...
        } catch (err) { if (err.code !== "ENOENT") console.error(`[structure] readdir doc dir failed: ${entry.name}: ${err.message}`); }
      }
    }
  } catch (err) { if (err.code !== "ENOENT") console.error(`[structure] readdir project failed: ${projectPath}: ${err.message}`); }
...
async function generateProjectContext(projectPath) {
  ...
  async function collectDocFiles(dir, relative) {
    ...
    for (const entry of entries) {
      ...
      } catch (err) { if (err.code !== "ENOENT") console.error(`[structure] readFile failed: ${fullPath}: ${err.message}`); }
    }
  }
