...
const { DEFAULT_CONFIG } = require("./ucmd-constants.js");
...
const _defaultDeps = {
  log: _noop,
  broadcastWs: _noop,
  markStateDirty: _noop,
  config: () => ({}),
  daemonState: () => ({ stats: { totalSpawns: 0 } }),
  spawnAgent: _noopAsync,
  submitTask: _noopAsync,
};
let deps = { ..._defaultDeps };

function setDeps(d) {
  // Cancel all active sessions to prevent fire-and-forget leaks when deps are swapped (e.g. in tests)
  for (const [sid, session] of refinementSessions) {
    session.completed = true; // prevent further broadcastWs calls
    refinementSessions.delete(sid);
  }
  deps = { ..._defaultDeps, ...d };
}
...
const _pendingRefinements = new Set();
...
async function generateNextQuestion(sessionId) {
  ...
  deps.broadcastWs("refinement:status", {
    sessionId,
    status: "질문 생성 중...",
  });
  ...
  try {
    ...
    if (session._parseRetries < 2) {
      deps.log(
        "[refinement] JSON parse failed, retrying question generation...",
      );
  ...
  if (parsed.done) {
    session.coverage = computeCoverage(
      session.decisions,
      session.expectedAreas,
    );
    if (!isFullyCovered(session.coverage)) {
      session._doneRetries = (session._doneRetries || 0) + 1;
      if (session._doneRetries < 2) {
        deps.log(
          "[refinement] premature done before coverage complete, retrying question generation...",
        );
        return generateNextQuestion(sessionId);
      }
  ...
  const hasValidArea =
    normalizedArea &&
    Object.hasOwn(session.expectedAreas || {}, normalizedArea);
  if (!parsed.question || !parsed.options || !hasValidArea) {
    ...
    deps.broadcastWs("refinement:error", {
      sessionId,
      error: "invalid question format after retry",
    });
    clearRefinementSession(sessionId, session);
    return;
  }
...
async function runAutopilotRefinement(sessionId) {
  ...
  const config = deps.config();
  const cfg = config || DEFAULT_CONFIG;
  const requiredCoverageRounds = Object.values(
    session.expectedAreas || {},
  ).reduce(
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (result.status !== "done") {
      deps.broadcastWs("refinement:error", {
        sessionId,
        error: `autopilot round ${round + 1} failed`,
      });
      clearRefinementSession(sessionId, session);
      return;
    }
    ...
    try {
      ...
      parseFailures += 1;
      deps.log(
        `[refinement] JSON parse failed at round ${round + 1}, retrying...`,
      );
    ...
    if (parsed.done) {
      ...
      if (!isFullyCovered(session.coverage)) {
        deps.log(
          `[refinement] autopilot reported done before full coverage at round ${round + 1}, continuing...`,
        );
        continue;
      }
      if (parsed.pipeline) session.pipeline = parsed.pipeline;
    ...
    const updatedCoverage = computeCoverage(
      session.decisions,
      session.expectedAreas,
    );
  ...
  deps.broadcastWs("refinement:error", {
    sessionId,
    error:
      "autopilot reached max rounds before refinement coverage was complete",
  });
  clearRefinementSession(sessionId, session);
}
...
async function startRefinement(params = {}) {
  ...
  const normalizedDescriptionInput =
  ...
  const normalizedDescription =
    normalizedDescriptionInput || normalizedBodyInput;
  ...
  deps.log(
    `refinement started: ${sessionId} (mode=${session.mode}, brownfield=${isBrownfield})`,
  );
  ...
  _pendingRefinements.add(p);
  p.finally(() => _pendingRefinements.delete(p));
...
async function switchToAutopilot(sessionId) {
  ...
  if (session.completed === true)
    throw new Error("refinement already complete");
  if (session.mode === "autopilot")
    throw new Error("refinement already in autopilot mode");
  ...
  try {
    deps.broadcastWs("refinement:mode_changed", {
      sessionId,
      mode: "autopilot",
    });
  } catch (e) {
  ...
  const p = runAutopilotRefinement(sessionId).catch((e) => {
...
async function finalizeRefinement(sessionId) {
  const session = refinementSessions.get(sessionId);
  if (!session) throw new Error("session not found");
  if (session.finalizing === true)
    throw new Error("refinement finalization in progress");
  ...
  try {
    ...
    try {
      deps.broadcastWs("refinement:finalized", {
        sessionId,
        taskId: result.id,
      });
    } catch (broadcastError) {
      deps.log(
        `refinement finalized broadcast error: ${broadcastError.message}`,
      );
    }
...
async function _drainPending() {
  while (_pendingRefinements.size > 0) {
